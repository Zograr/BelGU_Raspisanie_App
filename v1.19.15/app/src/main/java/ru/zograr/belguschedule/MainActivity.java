...
public class MainActivity extends Activity {
            ...
            public void onPageFinished(WebView view, String url) {
                ...
                if (fixedUrl.contains("journal_spo")) {
                    ...
                    if ((fixedUrl.contains("login") || fixedUrl.contains("portfolio")) && pendingDekanatLogin.isEmpty()) {
                        addGradesMessage("Нужен вход", "Сессия сайта не активна или истекла. Нажми «Войти заново» и войди ещё раз.");
                    }
                }
            }
    ...
    private void addGradeRowCard(JSONObject row) {
        ...
        String typeLabel = gradeTypeLabel(row.optString("type", ""));
        if (!typeLabel.isEmpty()) {
            addGradeChip(meta, "Формат: " + typeLabel);
        }

        String attendanceLabel = gradeTypeLabel(row.optString("attendance", ""));
        if (!attendanceLabel.isEmpty()) {
            addGradeChip(meta, "Посещение: " + attendanceLabel);
        }
        ...
        String typeDetails = gradeTypeLabel(row.optString("type", ""));
        if (!typeDetails.isEmpty()) {
            details.addView(gradesInfoLine("Формат", typeDetails));
        }

        String attendanceDetails = gradeTypeLabel(row.optString("attendance", ""));
        if (!attendanceDetails.isEmpty()) {
            details.addView(gradesInfoLine("Посещение", attendanceDetails));
        }
    ...
    private void addGradesSelectionStatus() {
        ...
        hint.setText("Перед сбором проверь семестр и неделю. Длинная неделя теперь не съезжает.");
        hint.setTextColor(textMuted);
        hint.setTextSize(12);
        LinearLayout.LayoutParams hintParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        hintParams.setMargins(0, dp(4), 0, dp(10));
    ...
    private void refreshGradesSelectionViews() {
        if (gradesSelectedSemesterView != null) {
            gradesSelectedSemesterView.setText("Семестр: " + (selectedGradesSemester == null || selectedGradesSemester.trim().isEmpty()
                    ? "не выбрано"
                    : selectedGradesSemester.trim()));
        }

        if (gradesSelectedWeekView != null) {
            gradesSelectedWeekView.setText("Неделя: " + (selectedGradesWeek == null || selectedGradesWeek.trim().isEmpty()
                    ? "не выбрано"
    ...
    private void addProblemRowCard(JSONObject row) {
        ...
        if (!row.optString("type", "").isEmpty()) {
            addGradeChip(chips, "тип: " + row.optString("type", ""));
        }
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.19.15");
