...
public class MainActivity extends Activity {
    ...
    private void refreshTeachersPanelContent(String query) {
        ...
        if (!teacherScheduleOpen) {
            ...
            LinearLayout linkButton = settingsButtonCard("Открыть по ссылке",
                    "Можно вставить ссылку вида teachers/index.php?teacher=123732");
            linkButton.setOnClickListener(v -> showTeacherLinkDialog());
            LinearLayout.LayoutParams linkParams = new LinearLayout.LayoutParams(
                    LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
            linkParams.setMargins(0, 0, 0, dp(12));
            teachersPanel.addView(linkButton, linkParams);
    ...
    private void showTeacherLinkDialog() {
        ...
        input.setSingleLine(true);
        input.setHint("https://bsuedu.ru/...teacher=123732");
        ...
        new AlertDialog.Builder(this)
                .setTitle("Открыть преподавателя")
                .setMessage("Вставь ссылку на расписание преподавателя с сайта БелГУ.")
                .setView(input)
                .setNegativeButton("Отмена", null)
                .setPositiveButton("Открыть", (dialog, which) -> {
                    String value = input.getText() == null ? "" : input.getText().toString().trim();
                    Matcher matcher = Pattern.compile("teacher=(\\d+)").matcher(value);
                    if (matcher.find()) {
                        openTeacherSchedule(new TeacherItem(matcher.group(1), "Преподаватель " + matcher.group(1)));
                    } else {
                        Toast.makeText(this, "В ссылке не найден teacher=ID", Toast.LENGTH_LONG).show();
                    }
                })
                .show();
    }
