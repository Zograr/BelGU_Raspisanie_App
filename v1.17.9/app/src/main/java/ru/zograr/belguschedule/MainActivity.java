...
public class MainActivity extends Activity {
    ...
    private void addGradesTools() {
        ...
        TextView sub = new TextView(this);
        sub.setText("Сайт работает скрыто внутри приложения. Выбери семестр и неделю кнопками ниже, затем нажми «Собрать оценки». Результат будет показан здесь крупными карточками.");
        sub.setTextColor(textMuted);
        sub.setTextSize(13);
    ...
    private void addGradeRowCard(JSONObject row) {
        ...
        String raw = row.optString("raw", "").trim();
        if (!raw.isEmpty()) {
            TextView rawView = gradesInfoLine("Источник", raw.length() > 160 ? raw.substring(0, 160) + "..." : raw);
            rawView.setTextSize(11);
            rawView.setAlpha(0.72f);
            LinearLayout.LayoutParams rawParams = new LinearLayout.LayoutParams(
                    LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
            rawParams.setMargins(0, dp(4), 0, 0);
            details.addView(rawView, rawParams);
        }
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.17.9.9.8.7.6.5.5.4.3.2");
