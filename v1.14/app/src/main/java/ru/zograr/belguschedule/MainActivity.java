...
public class MainActivity extends Activity {
    ...
    private void addThemeButton() {
        ...
        subtitle.setText("Сейчас: " + themeDisplayName(themeMode));
        subtitle.setTextColor(textMuted);
        subtitle.setTextSize(13);
        LinearLayout.LayoutParams subtitleParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        subtitleParams.setMargins(0, dp(4), 0, dp(10));
        card.addView(subtitle, subtitleParams);

        addThemeOptionRow(card, "green_light", "Зелёная светлая", "мягкая классическая");
        addThemeOptionRow(card, "green_dark", "Зелёная тёмная", "как раньше, но темнее");
        addThemeOptionRow(card, "blue_light", "Синяя светлая", "спокойный учебный стиль");
        addThemeOptionRow(card, "blue_dark", "Синяя тёмная", "ночной синий");
        addThemeOptionRow(card, "purple_light", "Фиолетовая светлая", "более мягкий акцент");
        addThemeOptionRow(card, "purple_dark", "Фиолетовая тёмная", "контрастная тёмная");
        addThemeOptionRow(card, "amber_light", "Тёплая светлая", "бежево-оранжевая");
        addThemeOptionRow(card, "amber_dark", "Тёплая тёмная", "тёмная янтарная");

        addSettingsCard(card);
    }

    private void addThemeOptionRow(LinearLayout parent, String mode, String titleText, String subtitleText) {
        ...
        row.setBackground(locationBackground(active ? greenSoft : (darkMode ? Color.rgb(24, 30, 26) : Color.rgb(250, 252, 250)), active ? green : border));
        row.setOnClickListener(v -> setThemeMode(mode));
        ...
        row.addView(dot, new LinearLayout.LayoutParams(dp(30), LinearLayout.LayoutParams.WRAP_CONTENT));
        ...
        preview.setPadding(dp(10), dp(5), dp(10), dp(5));
...
private void addWeekPairRow(LinearLayout parent, String pairText, boolean isToday) {
    ...
    row.setBackground(locationBackground(
            currentPair ? (darkMode ? Color.rgb(20, 55, 31) : Color.rgb(226, 247, 231)) : (darkMode ? Color.rgb(24, 29, 26) : Color.rgb(250, 252, 250)),
...
private void addCompactPairCard(LinearLayout parent, String pairText) {
    ...
    item.setBackground(currentPair
            ? locationBackground(darkMode ? Color.rgb(20, 55, 31) : Color.rgb(226, 247, 231), green)
            : cardBackground(darkMode ? Color.rgb(24, 29, 26) : Color.rgb(248, 251, 248)));
