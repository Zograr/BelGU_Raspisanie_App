...
public class MainActivity extends Activity {
    ...
    private void addTeacherScheduleActionButtons(TeacherItem teacher) {
        ...
        TextView refresh = teacherActionButton("Обновить с сайта");
        refresh.setOnClickListener(v -> refreshTeacherScheduleFromSite(teacher));
        row.addView(refresh, new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1));

        TextView next = teacherActionButton("Следующая неделя");
        ...
        next.setOnClickListener(v -> loadNextTeacherWeek(teacher));
    ...
    private void loadNextTeacherWeek(TeacherItem teacher) {
        ...
        if (teachersPanelContent != null) {
            teachersPanelContent.removeAllViews();
            addTeacherMessage("Открываю следующую неделю",
                    resolved.name + "\nНажимаю «Следующая неделя» на странице БелГУ...");
        }

        try {
            hiddenWebView.evaluateJavascript(buildTeacherNextWeekScript(), value -> {
                main.postDelayed(() -> {
            ...
            if (teachersPanelContent != null) {
                teachersPanelContent.removeAllViews();
                addTeacherMessage("Не удалось открыть следующую неделю",
                        "Ошибка: " + e.getMessage());
                addTeacherScheduleActionButtons(resolved);
            }
        }
    ...
    private String buildTeacherNextWeekScript() {
        return "(function(){" +
                ...
                "if(t.indexOf('следующая неделя')>=0||t.indexOf('след.')>=0){" +
                "try{e.click();return JSON.stringify({ok:true});}catch(ex){}" +
                "}" +
                "}" +
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.16.1");
