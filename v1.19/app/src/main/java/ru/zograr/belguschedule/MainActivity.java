...
public class MainActivity extends Activity {
    ...
    private void addGradesLoginForm() {
        ...
        note.setText("Пароль не сохраняется в приложении. Для удобства используй системный менеджер паролей/автозаполнение Android. Галочка «Не выходить» сохраняет cookies сайта, а кнопку «Продлить сессию» можно нажимать до истечения 12 часов.");
    ...
    private void addGradesSessionCard() {
        SharedPreferences prefs = getSharedPreferences(PREFS, MODE_PRIVATE);
        boolean keepSession = prefs.getBoolean(KEY_DEKANAT_KEEP_SESSION, false);

        LinearLayout card = settingsCard();
        card.setOrientation(LinearLayout.VERTICAL);

        TextView title = new TextView(this);
        title.setText("Сессия личного кабинета");
        title.setTextColor(textMain);
        title.setTextSize(16);
        title.setTypeface(Typeface.DEFAULT_BOLD);
        card.addView(title);
        ...
        status.setText(dekanatSessionStatusText(keepSession));
        status.setTextColor(textMuted);
        status.setTextSize(13);
        status.setLineSpacing(dp(2), 1.0f);
        LinearLayout.LayoutParams statusParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        statusParams.setMargins(0, dp(7), 0, 0);
        card.addView(status, statusParams);

        TextView extend = gradesSmallButton("Продлить сессию");
        extend.setOnClickListener(v -> refreshDekanatSession());
        LinearLayout.LayoutParams extendParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        extendParams.setMargins(0, dp(10), 0, 0);
        card.addView(extend, extendParams);

        addGradesCard(card);
    }

    private String dekanatSessionStatusText(boolean keepSession) {
        if (!keepSession) {
            return "Автовход выключен. Включи галочку «Не выходить из системы» при следующем входе.";
        }

        long saved = getSharedPreferences(PREFS, MODE_PRIVATE).getLong(KEY_DEKANAT_SESSION_TIME, 0L);
        if (saved <= 0) {
            return "Сессия ещё не подтверждена. После открытия журнала приложение запомнит время входа.";
        }

        long age = Math.max(0L, System.currentTimeMillis() - saved);
        long left = DEKANAT_SESSION_MAX_MS - age;

        if (left <= 0) {
            return "Сессия могла истечь. Нажми «Продлить сессию» — если сайт ещё помнит cookies, вход обновится. Если нет, понадобится войти заново.";
        }

        long hours = left / (60L * 60L * 1000L);
        long minutes = (left / (60L * 1000L)) % 60L;

        String hint = age >= DEKANAT_SESSION_REFRESH_MS
                ? "\nЛучше нажать «Продлить сессию», пока сайт ещё не выкинул из аккаунта."
                : "\nМожно не вводить пароль, пока cookies сайта живы.";

        return "Примерно осталось: " + hours + " ч " + minutes + " мин." + hint;
    }

    private void refreshDekanatSession() {
        ensureGradesWebView();
        gradesJournalReady = false;
        addGradesMessage("Продлеваю сессию",
                "Открываю журнал скрыто. Если cookies ещё живые, время сессии обновится без ввода пароля.");
            ...
            public void onPageFinished(WebView view, String url) {
                ...
                if (fixedUrl.contains("journal_spo")) {
                    ...
                    getSharedPreferences(PREFS, MODE_PRIVATE)
                            ...
                            .apply();
                    addGradesMessage("Журнал готов", "Теперь можно выбрать семестр и неделю.");
                } else {
                    gradesJournalReady = false;
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.19");
