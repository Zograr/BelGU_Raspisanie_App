...
public class MainActivity extends Activity {
    ...
    private String buildCollectGradesScript() {
        return "(function(){" +
                ...
                "function normToken(s){return clean(s).toLowerCase().replace(/[.:]/g,'').trim();}" +
                "function splitTokens(s){return clean(s).split(/[\\s,;|\\/\\\\]+/).map(normToken).filter(Boolean);}" +
                "function isGradeToken(t){t=normToken(t);return /^(нн|н|2|3|4|5|зач|незач|з)$/.test(t);}" +
                "function isPresenceToken(t){t=normToken(t);return /^(п|о)$/.test(t);}" +
                "function isLessonTypeToken(t){t=normToken(t);return /^(л|лек|лаб|пр|сем|пз|прз)$/.test(t);}" +
                "function normalizeGrade(t){t=normToken(t);if(t==='н'||t==='нн')return t.toUpperCase();if(t==='з')return 'зач';return t;}" +
                "function isHeader(s){s=clean(s).toLowerCase();return !s||s==='п'||s==='о'||s==='у'||s==='тема'||s.indexOf('выберите')>=0||s.indexOf('журнал')>=0;}" +
                "function badSubject(s){s=clean(s);var low=s.toLowerCase();var toks=splitTokens(s);return isHeader(s)||isDate(s)||dayName(s)||isPair(s)||toks.every(function(x){return isGradeToken(x)||isPresenceToken(x)||isLessonTypeToken(x);})||low==='дата'||low==='день';}" +
                "function badTheme(s){s=clean(s);return badSubject(s)||s.toLowerCase()==='тема';}" +
                "function unique(arr){var out=[];arr.forEach(function(x){if(x&&out.indexOf(x)<0)out.push(x);});return out;}" +
                "function firstAfterSubject(raw, subject){" +
                "var pos=raw.indexOf(subject);var tail=pos>=0?raw.slice(pos+subject.length):raw;" +
                "tail=tail.replace(/\\d{2}\\.\\d{2}\\.\\d{4}/g,' ');" +
                "var toks=splitTokens(tail);" +
                "var marks=[];var pres='';var lessonType='';" +
                "for(var i=0;i<toks.length;i++){var t=toks[i];if(isGradeToken(t)){marks.push(normalizeGrade(t));}else if(isPresenceToken(t)&&!pres){pres=t.toUpperCase();}else if(isLessonTypeToken(t)&&!lessonType){lessonType=t;}}" +
                "marks=unique(marks);return {mark:marks.join(', '),presence:pres,type:lessonType};" +
                "}" +
                ...
                "if(!subject)return;" +
                "var extracted=firstAfterSubject(raw,subject);" +
                "var theme='';" +
                "for(var i=cells.length-1;i>subjectIndex;i--){var tc=cells[i];if(!badTheme(tc)&&tc!==subject&&/[А-Яа-яA-Za-z]/.test(tc)){theme=tc;break;}}" +
                "var attendance=(extracted.mark.indexOf('НН')>=0?'НН':(extracted.mark.indexOf('Н')>=0?'Н':extracted.presence));" +
                "out.push({day:currentDay,date:currentDate,lesson:cells[pairIndex],subject:subject,type:extracted.type,mark:extracted.mark,attendance:attendance,theme:theme,raw:raw});" +
                "});" +
    ...
    private String displayGradeValue(JSONObject row) {
        ...
        String attendance = row.optString("attendance", "").trim();

        String fromMark = firstProblemOrGradeToken(mark);
        if (!fromMark.isEmpty()) return fromMark;

        String fromAttendance = firstAbsenceToken(attendance);
    ...
    private String firstGradeTokenFromRaw(JSONObject row) {
        ...
        // Убираем дату, чтобы не ловить цифры из 2026.
        tail = tail.replaceAll("\\d{2}\\.\\d{2}\\.\\d{4}", " ");

        String cleaned = tail.replace(',', ' ').replace(';', ' ').replace('|', ' ')
                .replace('/', ' ').replace('\\', ' ');
    ...
    private String attendanceCode(JSONObject row) {
        ...
        String direct = normalizeGradePart(attendance);
        if (direct.equals("Н") || direct.equals("НН")) return direct;

        String lower = attendance.toLowerCase(ru).trim();
        if (lower.equals("п")) return "П";
        if (lower.equals("о")) return "О";

        // П/О пробуем брать из raw после предмета, чтобы не показывать длинный текст в чипе.
        ...
        String[] parts = tail.replace(',', ' ').replace(';', ' ').replace('|', ' ').split("\\s+");
        for (String part : parts) {
            String token = normalizeGradePart(part);
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.19.3");
