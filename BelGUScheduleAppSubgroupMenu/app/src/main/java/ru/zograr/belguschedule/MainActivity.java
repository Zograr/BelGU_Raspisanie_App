...
public class MainActivity extends Activity {
    ...
    private void addLocationBlock(LinearLayout parent, String line) {
        ...
        if (!room.isEmpty()) {
            addLocationChip(box, "Аудитория: " + room, greenDark, greenSoft, Color.rgb(187, 224, 194));
        }
        if (!building.isEmpty()) {
            addLocationChip(box, formatBuilding(building), Color.WHITE, greenDark, greenDark);
        }
    }

    private void addLocationChip(LinearLayout parent, String text, int textColor, int bgColor, int strokeColor) {
        ...
        chip.setBackground(pillBackground(bgColor, strokeColor));
    ...
    private String formatBuilding(String building) {
        String value = building.replaceAll("(?i)учебный\\s+корпус", "Корпус")
                .replaceAll("№\\s+", "№")
                .replaceAll("\\s+", " ")
                .trim();
        if (!value.toLowerCase(ru).startsWith("корпус")) {
            value = "Корпус: " + value;
        }
        return value;
    }
