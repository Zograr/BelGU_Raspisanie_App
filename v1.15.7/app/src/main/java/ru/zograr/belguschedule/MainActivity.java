...
public class MainActivity extends Activity {
    ...
    private static final String[] TEACHER_LINKS_FROM_CHAT = new String[] {
            // Сюда в следующих версиях я буду добавлять ссылки, которые ты скинешь в чат.
            // Пример: "https://bsuedu.ru/bsu/education/schedule/teachers/index.php?teacher=123732"
    };
    ...
    private void refreshTeachersPanelContent(String query) {
        ...
        if (!teacherScheduleOpen) {
            ...
            LinearLayout siteSearchButton = settingsButtonCard("Найти на сайте БелГУ",
                    "Работает как поиск на сайте: вводишь фамилию и нажимаешь поиск");
            siteSearchButton.setOnClickListener(v -> {
                String value = search.getText() == null ? "" : search.getText().toString();
                performTeacherSiteSearch(value);
            });
            LinearLayout.LayoutParams siteSearchParams = new LinearLayout.LayoutParams(
                    LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
            siteSearchParams.setMargins(0, 0, 0, dp(12));
            teachersPanel.addView(siteSearchButton, siteSearchParams);
    ...
    private void performTeacherSiteSearch(String query) {
        String q = query == null ? "" : query.trim();
        if (q.length() < 2) {
            Toast.makeText(this, "Введи хотя бы 2 буквы фамилии", Toast.LENGTH_SHORT).show();
            return;
        }

        if (teachersPanelContent != null) {
            teachersPanelContent.removeAllViews();
            addTeacherMessage("Ищу на сайте БелГУ", "Запрос: " + q);
        }

        new Thread(() -> {
            try {
                List<TeacherItem> found = findTeachersBySiteSearch(q);
                main.post(() -> showTeacherSearchResults(q, found));
            } catch (Exception e) {
                main.post(() -> {
                    if (teachersPanelContent != null) {
                        teachersPanelContent.removeAllViews();
                        addTeacherMessage("Не удалось выполнить поиск", e.getMessage() == null ? "Сайт не ответил на поиск." : e.getMessage());
                    }
                });
            }
        }).start();
    ...
    private void showTeacherSearchResults(String query, List<TeacherItem> found) {
        if (teachersPanelContent == null) return;
        teachersPanelContent.removeAllViews();

        if (found == null || found.isEmpty()) {
            addTeacherMessage("Ничего не найдено", "По запросу «" + query + "» сайт не вернул преподавателей. Попробуй ввести только фамилию.");
            return;
        }

        TextView header = new TextView(this);
        header.setText("Найдено: " + found.size());
        header.setTextColor(textMuted);
        header.setTextSize(13);
        LinearLayout.LayoutParams headerParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        headerParams.setMargins(0, 0, 0, dp(8));
        teachersPanelContent.addView(header, headerParams);

        for (TeacherItem item : found) {
            addTeacherToCacheIfMissing(item);
            addTeacherCard(item);
        }
    }

    private List<TeacherItem> findTeachersBySiteSearch(String query) throws Exception {
        List<TeacherItem> result = new ArrayList<>();
        String encoded = java.net.URLEncoder.encode(query, "UTF-8");

        // Пробуем несколько вариантов параметров, потому что форма поиска БелГУ
        // может менять внутреннее имя поля.
        String[] urls = new String[] {
                "https://bsuedu.ru/bsu/education/schedule/?search=" + encoded,
                "https://bsuedu.ru/bsu/education/schedule/?q=" + encoded,
                "https://bsuedu.ru/bsu/education/schedule/?query=" + encoded,
                "https://bsuedu.ru/bsu/education/schedule/?text=" + encoded,
                "https://bsuedu.ru/bsu/education/schedule/?fio=" + encoded,
                "https://bsuedu.ru/bsu/education/schedule/?search_text=" + encoded,
                TEACHERS_URL + "?search=" + encoded,
                TEACHERS_URL + "?q=" + encoded,
                TEACHERS_URL + "?query=" + encoded,
                TEACHERS_URL + "?text=" + encoded,
                TEACHERS_URL + "?fio=" + encoded,
                TEACHERS_URL + "?search_text=" + encoded,
                SCHEDULE_BASE_URL + scheduleGroup + "&search=" + encoded,
                SCHEDULE_BASE_URL + scheduleGroup + "&q=" + encoded
        };

        for (String url : urls) {
            try {
                String html = fetchUrl(url);
                collectTeachersFromHtml(result, html);
                collectTeacherLinksFromGroupHtml(result, html);
                collectTeacherFromSchedulePage(result, url, html);
            } catch (Exception ignored) {
            }
        }

        // Если сайт не отдал результаты через форму, всё равно ищем по HTML группы
        // и по уже сохранённому списку.
        try {
            String groupHtml = fetchUrl(SCHEDULE_BASE_URL + scheduleGroup);
            collectTeacherLinksFromGroupHtml(result, groupHtml);
            collectTeachersFromHtml(result, groupHtml);
        } catch (Exception ignored) {
        }

        addTeachersFromCachedGroupSchedule(result);

        List<TeacherItem> filtered = filterTeacherItems(result, query);
        java.util.Collections.sort(filtered, (a, b) -> a.name.compareToIgnoreCase(b.name));
        return filtered;
    }

    private void collectTeacherFromSchedulePage(List<TeacherItem> result, String sourceUrl, String html) {
        if (html == null) return;

        String id = "";
        Matcher idMatcher = Pattern.compile("teacher=(\\d+)").matcher(sourceUrl + " " + html);
        if (idMatcher.find()) {
            id = idMatcher.group(1);
        }
        if (id.isEmpty()) return;

        String text = stripHtml(html).replaceAll("\\s+", " ").trim();
        String name = "";

        Matcher namedTeacher = Pattern.compile("Преподаватель(?:\\s+[^А-ЯЁ]{0,20})?\\s+([А-ЯЁ][а-яё]+\\s+[А-ЯЁ][а-яё]+\\s+[А-ЯЁ][а-яё]+)").matcher(text);
        if (namedTeacher.find()) {
            name = namedTeacher.group(1).trim();
        }

        if (name.isEmpty()) {
            Matcher anyFullName = Pattern.compile("([А-ЯЁ][а-яё]+\\s+[А-ЯЁ][а-яё]+\\s+[А-ЯЁ][а-яё]+)").matcher(text);
            if (anyFullName.find()) {
                name = anyFullName.group(1).trim();
            }
        }
    ...
    private List<TeacherItem> filterTeacherItems(List<TeacherItem> items, String query) {
        List<TeacherItem> filtered = new ArrayList<>();
        String q = query == null ? "" : query.trim().toLowerCase(ru);

        for (TeacherItem item : items) {
            if (item == null || item.name == null) continue;
            if (q.isEmpty() || item.name.toLowerCase(ru).contains(q)) {
                addTeacherItemUnique(filtered, item);
            }
        }

        if (filtered.isEmpty() && q.length() >= 4) {
            for (TeacherItem item : items) {
                if (item == null || item.name == null) continue;
                String lower = item.name.toLowerCase(ru);
                if (lower.startsWith(q) || lower.contains(q.substring(0, Math.min(q.length(), 4)))) {
                    addTeacherItemUnique(filtered, item);
                }
            }
        }
    ...
    private void addTeacherItemUnique(List<TeacherItem> list, TeacherItem candidate) {
        for (TeacherItem item : list) {
            if (item.id.equals(candidate.id) || item.name.equalsIgnoreCase(candidate.name)) {
                return;
            }
        }
        list.add(candidate);
    ...
    private void addTeacherToCacheIfMissing(TeacherItem candidate) {
        if (candidate == null) return;
        for (TeacherItem item : teacherItems) {
            if (item.id.equals(candidate.id) || item.name.equalsIgnoreCase(candidate.name)) {
                return;
            }
        }
        teacherItems.add(candidate);
    }

    private void loadTeacherList() {
        if (teachersPanelContent != null) {
            teachersPanelContent.removeAllViews();
            addTeacherMessage("Загружаю список", "Получаю известных преподавателей и готовлю поиск по сайту БелГУ...");
        }
    ...
    private List<TeacherItem> fetchTeacherList() throws Exception {
        ...
        // 1) Берём преподавателей со страницы выбранной группы, потому что там уже
        // есть рабочие ссылки teachers/index.php?teacher=ID.
        try {
            String groupHtml = fetchUrl(SCHEDULE_BASE_URL + scheduleGroup);
            collectTeacherLinksFromGroupHtml(result, groupHtml);
            collectTeachersFromHtml(result, groupHtml);
        ...
        // 1.5) Ссылки, которые ты будешь скидывать мне в чат, будут добавлены сюда
        // прямо в код приложения и появятся в списке без ручного ввода в приложении.
        collectTeacherLinksFromChat(result);

        // 2) Пытаемся получить более общий список с раздела преподавателей и похожих
        // страниц. Если сайт отдаст варианты select/option или ссылки, добавим их тоже.
        String[] sources = new String[] {
                TEACHERS_URL,
                TEACHERS_URL + "index.php",
                TEACHERS_URL + "index.php?teacher=1698",
                TEACHERS_URL + "index.php?teacher=123732",
                "https://www.bsuedu.ru/bsu/education/schedule/teachers/",
                "https://m.bsu.edu.ru/bsu/education/schedule/teachers/",
                "https://ftp.bsu.edu.ru/bsu/education/schedule/teachers/",
                SCHEDULE_BASE_URL + scheduleGroup
        };

        for (String source : sources) {
            try {
                String html = fetchUrl(source);
                collectTeachersFromHtml(result, html);
                collectTeacherLinksFromGroupHtml(result, html);
            } catch (Exception ignored) {
            }
        ...
        // 3) Запасной вариант: если ID преподавателя сайт не отдал, берём имена из
        // сохранённого расписания группы. Такие карточки помечаются отдельно.
        addTeachersFromCachedGroupSchedule(result);

        java.util.Collections.sort(result, (a, b) -> a.name.compareToIgnoreCase(b.name));
        if (result.isEmpty()) {
            throw new Exception("Список преподавателей не найден. Обнови расписание группы и попробуй ещё раз.");
        }
        return result;
    }

    private void collectTeacherLinksFromChat(List<TeacherItem> result) {
        for (String link : TEACHER_LINKS_FROM_CHAT) {
            if (link == null || link.trim().isEmpty()) continue;
            Matcher matcher = Pattern.compile("teacher=(\\d+)").matcher(link);
            if (matcher.find()) {
                String id = matcher.group(1);
                addTeacherIfValid(result, id, "Преподаватель " + id, false);
            }
        }
    }
    ...
    private void openTeacherSchedule(TeacherItem teacher) {
        ...
        if (teacher.id.startsWith("cached:")) {
            showCachedTeacherSchedule(teacher);
            return;
        }
    ...
    private void showTeacherScheduleText(TeacherItem teacher, String scheduleText) {
        ...
        TextView note = new TextView(this);
        note.setText("Полное расписание преподавателя с сайта: предмет, тип занятия, группа, аудитория и корпус.");
        note.setTextColor(textMuted);
        note.setTextSize(13);
        LinearLayout.LayoutParams noteParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        ...
        List<DaySchedule> days = splitDays(scheduleText == null ? "" : scheduleText);
        int total = 0;

        for (DaySchedule day : days) {
            if (day.pairs == null || day.pairs.isEmpty()) continue;
            addTeacherScheduleDayBlock(day);
            total += day.pairs.size();
        }

        if (total == 0) {
            List<String> pairs = splitPairs(scheduleText == null ? "" : scheduleText);
            if (!pairs.isEmpty()) {
                DaySchedule fakeDay = new DaySchedule("Неделя", "Расписание", scheduleText, pairs, false);
                addTeacherScheduleDayBlock(fakeDay);
                total = pairs.size();
            }
        }
        ...
        if (total == 0) {
            ...
            body.setText(scheduleText == null || scheduleText.trim().isEmpty()
                    ...
                    : cleanupTeacherScheduleRawText(scheduleText));
