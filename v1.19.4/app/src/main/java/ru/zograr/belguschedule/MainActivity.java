...
public class MainActivity extends Activity {
    ...
    private void addGradesTools() {
        ...
        TextView sub = new TextView(this);
        sub.setText("Выбери семестр и нажми «Подгрузить семестр». Если нужна одна неделя — выбери неделю, она загрузится автоматически.");
        sub.setTextColor(textMuted);
        sub.setTextSize(13);
        ...
        TextView loadSemester = gradesBigButton("Подгрузить семестр");
    ...
    private String buildGetWeekOptionsScript() {
        return "(function(){" +
                ...
                "return JSON.stringify([].slice.call(best.options).map(function(o,i){return {index:i,text:txt(o)||('Неделя '+(i+1))};}).filter(function(x){return x.text&&x.text.toLowerCase().indexOf('выберите')<0&&!/^[1-8]$/.test(x.text)&&!isLang(x.text);}));" +
                "})()";
    }
    ...
    private void loadGradesForSelectedSemester() {
        ...
        addGradesMessage("Подгружаю семестр", "Собираю все недели выбранного семестра...");
            ...
            try {
                ...
                if (weeks.length() == 0) {
                    addGradesMessage("Недели не найдены",
                            "Подожди пару секунд после выбора семестра и нажми «Подгрузить семестр» ещё раз.");
                    return;
                }
    ...
    private void loadNextSemesterWeek() {
        if (gradesSemesterWeekIndex >= gradesSemesterWeeks.length()) {
            ...
            Toast.makeText(this, "Семестр подгружен. Записей: " + gradesSemesterRows.length(), Toast.LENGTH_LONG).show();
            return;
        }
    ...
    private void collectGradesFromJournal() {
        ...
        return "(function(){" +
                ...
                "function push(a,s){s=clean(s);if(s&&a.indexOf(s)<0)a.push(s);}" +
                "function cellText(td){var a=[];push(a,td.innerText);push(a,td.textContent);push(a,td.value);push(a,td.title);push(a,td.alt);push(a,td.getAttribute&&td.getAttribute('aria-label'));push(a,td.getAttribute&&td.getAttribute('data-original-title'));[].slice.call(td.querySelectorAll('*')).forEach(function(n){push(a,n.innerText);push(a,n.textContent);push(a,n.value);push(a,n.title);push(a,n.alt);push(a,n.className&&n.className.baseVal);push(a,n.className);push(a,n.id);push(a,n.name);push(a,n.getAttribute&&n.getAttribute('aria-label'));push(a,n.getAttribute&&n.getAttribute('data-original-title'));push(a,n.getAttribute&&n.getAttribute('data-title'));});return clean(a.join(' '));}" +
                ...
                "function isPair(s){s=clean(s).toLowerCase();var m=s.match(/^(\\d)(\\s*пара)?$/);if(!m)return false;var n=parseInt(m[1],10);return n>=1&&n<=8;}" +
                "function norm(t){return clean(t).toLowerCase().replace(/[.:()\\[\\]{}]/g,'').trim();}" +
                "function toks(s){return clean(s).replace(/\\d{2}\\.\\d{2}\\.\\d{4}/g,' ').split(/[\\s,;|\\/\\\\_\\-]+/).map(norm).filter(Boolean);}" +
                "function isProblem(t){t=norm(t);return t==='2'||t==='н'||t==='нн'||t==='n'||t==='nn'||t==='незач';}" +
                "function isGrade(t){t=norm(t);return t==='2'||t==='3'||t==='4'||t==='5'||t==='з'||t==='зач'||t==='незач'||t==='н'||t==='нн'||t==='n'||t==='nn';}" +
                "function isPresence(t){t=norm(t);return t==='п'||t==='о';}" +
                "function isType(t){t=norm(t);return t==='л'||t==='лек'||t==='лаб'||t==='пр'||t==='сем'||t==='пз'||t==='прз';}" +
                "function markName(t){t=norm(t);if(t==='н'||t==='n')return 'Н';if(t==='нн'||t==='nn')return 'НН';if(t==='з')return 'зач';return t;}" +
                "function onlyService(s){var ts=toks(s);return ts.length>0&&ts.every(function(x){return isGrade(x)||isPresence(x)||isType(x)||/^\\d+$/.test(x);});}" +
                "function badSubject(s){var c=clean(s),l=c.toLowerCase();return !c||isDate(c)||dayName(c)||isPair(c)||onlyService(c)||l==='дата'||l==='день'||l==='тема'||l.indexOf('выберите')>=0||l.indexOf('журнал')>=0;}" +
                "function unique(a){var o=[];a.forEach(function(x){if(x&&o.indexOf(x)<0)o.push(x);});return o;}" +
                "function detectFromCells(cells,startIndex){" +
                "var problems=[];var grades=[];var presence='';var type='';" +
                "for(var i=startIndex;i<cells.length;i++){" +
                "var ts=toks(cells[i]);" +
                "for(var j=0;j<ts.length;j++){" +
                "var t=ts[j];" +
                "if(isProblem(t)){problems.push(markName(t));}" +
                "else if(isGrade(t)){grades.push(markName(t));}" +
                "else if(isPresence(t)&&!presence){presence=t.toUpperCase();}" +
                "else if(isType(t)&&!type){type=t;}" +
                "}" +
                "}" +
                "problems=unique(problems);grades=unique(grades);" +
                "var mark=problems.length?problems.join(', '):grades.join(', ');" +
                "var attendance='';if(mark.indexOf('НН')>=0)attendance='НН';else if(mark.indexOf('Н')>=0)attendance='Н';else attendance=presence;" +
                "return {mark:mark,attendance:attendance,type:type};" +
                "}" +
                "var out=[];var currentDate='';var currentDay='';" +
                "[].slice.call(document.querySelectorAll('tr')).forEach(function(tr){" +
                "var tds=[].slice.call(tr.querySelectorAll('th,td'));var cells=tds.map(cellText).filter(Boolean);if(cells.length<2)return;" +
                "var raw=clean(cells.join(' | '));if(raw.toLowerCase().indexOf('выберите')>=0)return;" +
                "for(var d=0;d<cells.length;d++){if(isDate(cells[d])){currentDate=dateOf(cells[d]);break;}}" +
                "for(var dd=0;dd<cells.length;dd++){var dn=dayName(cells[dd]);if(dn){currentDay=dn;break;}}" +
                "var pairIndex=-1;for(var p=0;p<cells.length;p++){if(isPair(cells[p])){pairIndex=p;break;}}if(pairIndex<0)return;" +
                "var subject='';var subjectIndex=-1;for(var i=pairIndex+1;i<cells.length;i++){if(!badSubject(cells[i])&&/[А-Яа-яA-Za-z]/.test(cells[i])){subject=cells[i];subjectIndex=i;break;}}" +
                "if(!subject)return;" +
                "var detected=detectFromCells(cells,pairIndex+1);" +
                "var theme='';for(var j=cells.length-1;j>subjectIndex;j--){var cc=cells[j];if(!badSubject(cc)&&cc!==subject&&/[А-Яа-яA-Za-z]/.test(cc)){theme=cc;break;}}" +
                "out.push({day:currentDay,date:currentDate,lesson:cells[pairIndex],subject:subject,type:detected.type,mark:detected.mark,attendance:detected.attendance,theme:theme,raw:raw,cells:cells.join(' || ')});" +
                "});" +
    ...
    private void showGradesRows(JSONArray rows) {
        ...
        for (int i = 0; i < rows.length(); i++) {
            try {
                JSONObject row = rows.getJSONObject(i);
                String grade = displayGradeValue(row);
                String attendance = row.optString("attendance", "").trim().toLowerCase(ru);

                if (isProblemRow(row) && (grade.equalsIgnoreCase("Н") || grade.equalsIgnoreCase("НН")
                        || attendance.equals("н") || attendance.equals("нн"))) {
                    absences++;
        ...
        TextView problemsButton = gradesSmallButton("Показать 2 / Н / НН из этого списка");
    ...
    private void showGradeProblems(JSONArray rows) {
        ...
        TextView subtitle = new TextView(this);
        subtitle.setText("Здесь только проблемы из уже собранного списка оценок.");
        subtitle.setTextColor(textMuted);
        subtitle.setTextSize(13);
    ...
    private boolean isProblemRow(JSONObject row) {
        return isProblemGradeValue(displayGradeValue(row));
    }

    private boolean isProblemGradeValue(String value) {
        String m = value == null ? "" : value.toLowerCase(ru).trim();
        return m.equals("2")
                || m.equals("н")
                || m.equals("нн")
                || m.contains("незач");
    ...
    private String displayGradeValue(JSONObject row) {
        String fromMark = firstProblemOrGradeToken(row.optString("mark", ""));
        if (!fromMark.isEmpty()) return fromMark;

        String fromAttendance = firstAbsenceToken(row.optString("attendance", ""));
        if (!fromAttendance.isEmpty()) return fromAttendance;

        String fromRaw = firstGradeTokenFromRaw(row);
        if (!fromRaw.isEmpty()) return fromRaw;

        return "";
    }

        private String firstGradeTokenFromRaw(JSONObject row) {
        String raw = row.optString("raw", "");
        String cells = row.optString("cells", "");
        String subject = row.optString("subject", "");

        String tail = raw == null ? "" : raw;

        if (subject != null && !subject.trim().isEmpty()) {
            int pos = tail.indexOf(subject);
            if (pos >= 0) {
                tail = tail.substring(pos + subject.length());
            }
        }

        String combined =
                row.optString("mark", "") + " " +
                row.optString("attendance", "") + " " +
                tail + " " +
                cells;

        return firstProblemOrGradeToken(combined);
    }


            private String firstProblemOrGradeToken(String value) {
        ...
        String lower = cleaned.toLowerCase(ru);

        if (java.util.regex.Pattern.compile("(?iu)(^|[^а-яa-z0-9])нн([^а-яa-z0-9]|$)").matcher(lower).find()) return "НН";
        if (java.util.regex.Pattern.compile("(?i)(^|[^a-z0-9])nn([^a-z0-9]|$)").matcher(lower).find()) return "НН";

        if (java.util.regex.Pattern.compile("(?iu)(^|[^а-яa-z0-9])н([^а-яa-z0-9]|$)").matcher(lower).find()) return "Н";
        if (java.util.regex.Pattern.compile("(?i)(^|[^a-z0-9])n([^a-z0-9]|$)").matcher(lower).find()) return "Н";

        if (java.util.regex.Pattern.compile("(?<!\\d)2(?!\\d)").matcher(lower).find()) return "2";
        if (java.util.regex.Pattern.compile("(?iu)(^|[^а-яa-z0-9])незач([^а-яa-z0-9]|$)").matcher(lower).find()) return "незач";

        if (java.util.regex.Pattern.compile("(?<!\\d)3(?!\\d)").matcher(lower).find()) return "3";
        if (java.util.regex.Pattern.compile("(?<!\\d)4(?!\\d)").matcher(lower).find()) return "4";
        if (java.util.regex.Pattern.compile("(?<!\\d)5(?!\\d)").matcher(lower).find()) return "5";

        if (java.util.regex.Pattern.compile("(?iu)(^|[^а-яa-z0-9])зач([^а-яa-z0-9]|$)").matcher(lower).find()) return "зач";
        ...
        return "";
            ...
            private String firstAbsenceToken(String value) {
        if (value == null) return "";
    ...
    private String attendanceCode(JSONObject row) {
        String direct = normalizeGradePart(row.optString("attendance", ""));
        if (direct.equals("Н") || direct.equals("НН")) return direct;

        String lower = row.optString("attendance", "").toLowerCase(ru).trim();
        if (lower.equals("п")) return "П";
        if (lower.equals("о")) return "О";
        ...
        if (subject != null && !subject.trim().isEmpty()) {
            int pos = tail.indexOf(subject);
            if (pos >= 0) tail = tail.substring(pos + subject.length());
        }
        ...
        for (String part : parts) {
            String token = normalizeGradePart(part);
            if (token.equals("Н") || token.equals("НН")) return token;
            if (token.equals("п")) return "П";
            if (token.equals("о")) return "О";
        }
    ...
    private int gradeColor(String mark) {
        ...
        if (m.equals("2") || m.contains("незач")) return Color.rgb(190, 65, 65);
        if (m.equals("3")) return Color.rgb(186, 133, 34);
        if (m.equals("4")) return Color.rgb(58, 125, 82);
        if (m.equals("5") || m.contains("зач")) return green;
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.19.4");
