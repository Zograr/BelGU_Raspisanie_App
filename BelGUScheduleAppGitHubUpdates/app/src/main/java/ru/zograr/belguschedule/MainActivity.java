...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        dayRow.setPadding(dp(12), 0, dp(12), dp(8));
        root.addView(dayRow, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT));

        dayMenuButton = new Button(this);
        dayMenuButton.setAllCaps(false);
        dayMenuButton.setTextSize(15);
        dayMenuButton.setTypeface(Typeface.DEFAULT_BOLD);
        dayMenuButton.setTextColor(greenDark);
        dayMenuButton.setBackground(buttonBackground(cardSurface, border));
        dayMenuButton.setOnClickListener(v -> showDayMenu());
        dayRow.addView(dayMenuButton, new LinearLayout.LayoutParams(0, dp(46), 1));
        ...
        updateButton.setTextColor(greenDark);
        updateButton.setBackground(buttonBackground(cardSurface, border));
        updateButton.setOnClickListener(v -> checkForUpdates(true));
        LinearLayout.LayoutParams updateParams = new LinearLayout.LayoutParams(dp(132), dp(46));
        updateParams.setMargins(dp(10), 0, 0, 0);
        dayRow.addView(updateButton, updateParams);
    ...
    private String cacheKey(String key) {
        return key + "_" + scheduleGroup + "_" + selectedDateString();
    }
    ...
    private void showDayMenu() {
        PopupMenu menu = new PopupMenu(this, dayMenuButton);
        String[] titles = {"Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота", "Воскресенье"};
        int today = currentDayIndex();
        for (int i = 0; i < titles.length; i++) {
            String label = titles[i] + " • " + dateForDayIndex(i);
            if (i == today) label = "Сегодня • " + titles[i] + " • " + dateForDayIndex(i);
            menu.getMenu().add(0, 100 + i, i, label);
        }
        menu.setOnMenuItemClickListener(item -> {
            selectDay(item.getItemId() - 100);
            return true;
        });
        menu.show();
    }
    ...
    private void selectDay(int dayIndex) {
        ...
        getSharedPreferences(PREFS, MODE_PRIVATE)
                .edit()
                .putInt(KEY_SELECTED_DAY, selectedDayIndex)
                .apply();
        updateSubgroupUi();
        listLayout.removeAllViews();
        summaryView.setText("Загружаю " + selectedDayNameShort().toLowerCase(ru) + "...");
        statusView.setText("Загружаю расписание за " + selectedDayNameShort().toLowerCase(ru) + "...");
        currentResult = null;
        loadCachedSchedule();
        refreshSchedule();
    }
    ...
    private void updateSubgroupUi() {
        ...
        if (dayMenuButton != null) {
            dayMenuButton.setText("День: " + selectedDayNameShort() + "  ▾");
        }
    }
    ...
    private void handleJsResult(String value, int attempt) {
        try {
            ...
            if (!debug.isEmpty()) {
                showLoadFailure("Не нашёл блок расписания за выбранный день. Сайт мог изменить разметку.");
            } else {
                showLoadFailure("Не нашёл пары за выбранный день. Возможно, в этот день занятий нет.");
            }
        } catch (Exception e) {
            if (attempt < MAX_EXTRACT_ATTEMPTS && !mainFrameError) {
                attemptExtract(attempt + 1);
    ...
    private String buildExtractScript() {
        String targetDate = selectedDateString();
        return "(function(){" +
                ...
                "if(!m){return JSON.stringify({ok:false,date:target,text:'',debug:body.slice(0,1200)});}" +
                "var start=m.index;" +
                ...
                "return JSON.stringify({ok:true,date:target,text:section,debug:body.slice(0,800)});" +
                "})()";
    }
    ...
    private ScheduleResult parseSchedule(String displayDate, String section) {
        ...
        if (pairs.isEmpty()) {
            if (looksLikeSchedule(clean)) {
                ...
                pairs.add("На выбранный день пары не найдены. Возможно, занятий нет или сайт временно отдал пустое расписание.");
            }
        }
    ...
    private void showSchedule(ScheduleResult result, boolean fromCache) {
        ...
        setLoading(false);
        titleView.setText("Расписание");
        dateView.setText(selectedDayNameShort() + " • " + result.displayDate);
        ...
        if (visiblePairs.isEmpty()) {
            addMessageCard("Пар не найдено", "Для выбранной подгруппы за " + selectedDayNameShort().toLowerCase(ru) + " пары не найдены. Можно переключиться на «Все», чтобы проверить полное расписание.");
        } else {
    ...
    private String summaryText(int pairCount) {
        String day = selectedDayNameShort();
        if (pairCount <= 0) return day + ": пары не найдены";
        if (pairCount == 1) return day + ": 1 пара";
        if (pairCount >= 2 && pairCount <= 4) return day + ": " + pairCount + " пары";
        return day + ": " + pairCount + " пар";
    }
