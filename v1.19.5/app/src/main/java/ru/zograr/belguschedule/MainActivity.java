...
public class MainActivity extends Activity {
    ...
    private void startQuickButtonShimmer(LinearLayout button, String title) {
        ...
        animator.addUpdateListener(animation -> {
            ...
            int bg = blendColor(quickButtonBg(title), cardSurface, darkMode ? 0.05f + value * 0.06f : 0.12f + value * 0.10f);
            int stroke = blendColor(quickButtonStroke(title), quickButtonAccent(title), 0.05f + value * 0.12f);
            button.setBackground(buttonBackground(bg, stroke));
        });
    ...
    private String buildCollectGradesScript() {
        return "(function(){" +
                ...
                "function cellText(td){var a=[];push(a,td.innerText);push(a,td.textContent);push(a,td.value);push(a,td.title);push(a,td.alt);push(a,td.getAttribute&&td.getAttribute('aria-label'));push(a,td.getAttribute&&td.getAttribute('data-original-title'));push(a,td.getAttribute&&td.getAttribute('data-title'));[].slice.call(td.querySelectorAll('input,select,textarea,option,span,a,div')).forEach(function(n){push(a,n.innerText);push(a,n.textContent);push(a,n.value);push(a,n.title);push(a,n.alt);push(a,n.getAttribute&&n.getAttribute('aria-label'));push(a,n.getAttribute&&n.getAttribute('data-original-title'));push(a,n.getAttribute&&n.getAttribute('data-title'));});return clean(a.join(' '));}" +
                ...
                "function toks(s){return clean(s).replace(/\\d{2}\\.\\d{2}\\.\\d{4}/g,' ').split(/[\\s,;|\\/\\\\]+/).map(norm).filter(Boolean);}" +
                "function isProblem(t){t=norm(t);return t==='2'||t==='н'||t==='нн'||t==='незач';}" +
                "function isGrade(t){t=norm(t);return t==='2'||t==='3'||t==='4'||t==='5'||t==='з'||t==='зач'||t==='незач'||t==='н'||t==='нн';}" +
                "function isPresence(t){t=norm(t);return t==='п'||t==='о';}" +
                "function isType(t){t=norm(t);return t==='л'||t==='лек'||t==='лаб'||t==='пр'||t==='сем'||t==='пз'||t==='прз';}" +
                "function markName(t){t=norm(t);if(t==='н')return 'Н';if(t==='нн')return 'НН';if(t==='з')return 'зач';return t;}" +
                ...
                "function detect(cells,startIndex){" +
                "var problems=[];var grades=[];var presence='';var type='';" +
                "for(var i=startIndex;i<cells.length;i++){" +
                "var c=clean(cells[i]);if(!c||isDate(c)||dayName(c))continue;" +
                "var ts=toks(c);" +
                ...
                "if(!subject)return;" +
                "var detected=detect(cells,subjectIndex+1);" +
    ...
    private String firstGradeTokenFromRaw(JSONObject row) {
        ...
        if (subject != null && !subject.trim().isEmpty()) {
            int pos = raw.indexOf(subject);
            if (pos >= 0) {
                tail = raw.substring(pos + subject.length());
            }
        }

        tail = tail.replaceAll("\\d{2}\\.\\d{2}\\.\\d{4}", " ");
        return firstProblemOrGradeToken(tail);
    }
    ...
    private String firstProblemOrGradeToken(String value) {
        ...
        for (String part : parts) {
            String token = normalizeGradePart(part);
            if (token.equals("3") || token.equals("4") || token.equals("5")
                    || token.equalsIgnoreCase("зач")) {
                return token;
            }
        }
    ...
    private String attendanceStatusText(String code) {
        if (code == null) return "";
    ...
    private void addAttendanceStatusCard(LinearLayout card, JSONObject row) {
        ...
        String status = attendanceStatusText(code);
        ...
        box.setBackground(pillBackground(greenSoft, border));
        ...
        LinearLayout.LayoutParams boxParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        boxParams.setMargins(0, dp(10), 0, 0);
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.19.5");
