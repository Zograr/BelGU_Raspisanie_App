...
public class MainActivity extends Activity {
    ...
    private void addSettingsVersionLabel() {
        TextView version = new TextView(this);
        version.setText("v1.11.2");
        version.setTextColor(darkMode ? Color.argb(95, 236, 242, 235) : Color.argb(90, 28, 94, 47));
        version.setTextSize(13);
        version.setGravity(Gravity.CENTER);
        version.setTypeface(Typeface.DEFAULT_BOLD);
        version.setPadding(0, dp(14), 0, dp(4));

        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        params.setMargins(0, dp(6), 0, 0);
        settingsPanelContent.addView(version, params);
    }
    ...
    private void downloadUpdateApk(String apkUrl, String tagName) {
        ...
        try {
            ...
            if (!downloadDir.exists() && !downloadDir.mkdirs()) {
                throw new Exception("Не удалось создать папку загрузки: " + downloadDir.getAbsolutePath());
            }

            File apkFile = new File(downloadDir, "BelGUScheduleApp-" + cleanFileName(tagName) + ".apk");
