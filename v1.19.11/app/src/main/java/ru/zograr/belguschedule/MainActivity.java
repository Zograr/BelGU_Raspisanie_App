...
public class MainActivity extends Activity {
    ...
    private void addGradesTools() {
        ...
        TextView sub = new TextView(this);
        sub.setText("Как в v1.17.14: для быстрого поиска проблем выбери семестр и нажми «Быстро найти 2 / Н / НН по семестру». Полная подгрузка всех оценок оставлена как эксперимент.");
        sub.setTextColor(textMuted);
        sub.setTextSize(13);
        ...
        TextView fastProblems = gradesBigButton("Быстро найти 2 / Н / НН по семестру");
        fastProblems.setOnClickListener(v -> collectAllGradeProblems());
        LinearLayout.LayoutParams fastProblemsParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        fastProblemsParams.setMargins(0, dp(10), 0, 0);
        actionsCard.addView(fastProblems, fastProblemsParams);

        TextView loadSemester = gradesSmallButton("Эксперимент: подгрузить все оценки");loadSemester.setOnClickListener(v -> loadGradesForSelectedSemester());
        ...
        TextView collectWeek = gradesSmallButton("Собрать неделю");
        ...
        TextView reset = gradesSmallButton("Сбросить выбор");
        ...
        LinearLayout.LayoutParams row3Params = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        row3Params.setMargins(0, dp(8), 0, 0);
        actionsCard.addView(row3, row3Params);

        TextView addCurrentWeek = gradesSmallButton("Добавить неделю в семестр");
        addCurrentWeek.setOnClickListener(v -> addCurrentWeekToSemesterManual());
        row3.addView(addCurrentWeek, new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1));

        TextView showManualSemester = gradesSmallButton("Показать собранный семестр");
        showManualSemester.setOnClickListener(v -> showManualSemesterRows());
        LinearLayout.LayoutParams showManualParams = new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1);
        showManualParams.setMargins(dp(8), 0, 0, 0);
        row3.addView(showManualSemester, showManualParams);

        TextView clearManualSemester = gradesSmallButton("Очистить собранный семестр");
        clearManualSemester.setOnClickListener(v -> clearManualSemesterRows());
        LinearLayout.LayoutParams clearManualParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        clearManualParams.setMargins(0, dp(8), 0, 0);
        actionsCard.addView(clearManualSemester, clearManualParams);
        ...
        if (gradesWebView == null || gradesWebView.getUrl() == null) {
            ensureGradesWebView();
            gradesJournalReady = false;
            addGradesMessage("Журнал загружается", "Подожди сообщение «Журнал готов», потом выбирай семестр/неделю.");
    ...
    private void chooseGradesOption(boolean semester) {
        ...
        if (!gradesJournalReady) {
            addGradesMessage("Журнал ещё загружается",
                    "Подожди сообщение «Журнал готов». Так мы не попадём в выбор языка RU/ENG.");
            return;
        }

        if (!semester && selectedGradesSemester.trim().isEmpty()) {
            addGradesMessage("Сначала семестр",
                    "Сначала выбери семестр, дождись обновления страницы, потом выбирай неделю.");
            return;
        }
            ...
            try {
                ...
                if (options.length() == 0) {
                    addGradesMessage("Список не найден", semester
                            ? "Не нашёл список семестров именно в журнале СПО. Подожди загрузку журнала."
                            : "Не нашёл список недель с датами. Подожди пару секунд после выбора семестра.");
                    return;
                }
    ...
    private String buildGetSemesterOptionsScript() {
        return "(function(){" +
                ...
                "return JSON.stringify([].slice.call(target.options).map(function(o,i){return {index:i,text:txt(o)||('Семестр '+(i+1))};}).filter(function(x){return /^[1-8]$/.test(x.text);}));" +
                "})()";
    }
    ...
    private void setGradesOption(boolean semester, int index, String label) {
        ...
        gradesWebView.evaluateJavascript(js, value -> {
            if (semester) {
                addGradesMessage("Семестр выбран",
                        label + "\nТеперь выбери неделю. После выбора недели оценки загрузятся автоматически.");
            } else {
                addGradesMessage("Неделя выбрана",
                        label + "\nПодгружаю оценки автоматически...");
                main.postDelayed(this::collectGradesFromJournal, 2600);
            }
        });
    }
    ...
    private void collectAllGradeProblems() {
        ...
        if (selectedGradesSemester == null || selectedGradesSemester.trim().isEmpty()) {
            addGradesMessage("Выбери семестр",
                    "Сначала выбери семестр, потом запускай быстрый поиск 2 / Н / НН.");
            return;
        }
    ...
    private void collectGradesFromJournal() {
        ...
        if (selectedGradesSemester == null || selectedGradesSemester.trim().isEmpty()
                || selectedGradesWeek == null || selectedGradesWeek.trim().isEmpty()) {
            addGradesMessage("Выбери семестр и неделю",
                    "Сначала выбери семестр и неделю, потом нажми «Собрать оценки».");
            return;
        }

        addGradesMessage("Собираю оценки", "Жду полную загрузку таблицы журнала...");
        collectGradesFromJournalRetry(0);
    }
    ...
    private void showGradesRows(JSONArray rows) {
        ...
        for (int i = 0; i < rows.length(); i++) {
            try {
                ...
                if (mark.isEmpty()) {
                    ...
                    withMarks++;
                    if (mark.equals("2") || mark.contains(", 2") || mark.startsWith("2,")) {
                        twos++;
                        problems++;
                    }
                }
        ...
        String extra = "";
        if (!selectedGradesSemester.isEmpty()) extra += "\nСеместр: " + selectedGradesSemester;
        if (!selectedGradesWeek.isEmpty()) extra += "\nНеделя: " + selectedGradesWeek;
        header.setText("Оценки и посещаемость" + extra);
        ...
        counts.setText("Записей: " + rows.length()
        ...
        TextView problemsButton = gradesSmallButton(problems > 0
                ? "Показать проблемы: 2 / Н / НН (" + problems + ")"
                : "Показать 2 / Н / НН");
        ...
        TextView debugButton = gradesSmallButton("Диагностика строк журнала");
        debugButton.setOnClickListener(v -> showGradesRawDiagnostics(rows));
        LinearLayout.LayoutParams debugParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        debugParams.setMargins(0, dp(8), 0, 0);
        summary.addView(debugButton, debugParams);
        ...
        if (rows.length() == 0) {
            ...
            emptyTitle.setText("Оценки не найдены");
            ...
            emptyText.setText("В таблице не удалось найти строки журнала. Проверь, выбран ли семестр и неделя.");
            ...
            gradesResultContent.addView(empty);
            return;
        }
    ...
    private void addGradeRowCard(JSONObject row) {
        ...
        String markText = displayGradeValue(row);
        TextView mark = new TextView(this);
        mark.setText(markText.isEmpty() ? "нет" : markText.toUpperCase(ru));
        mark.setTextColor(Color.WHITE);
        mark.setTextSize(markText.length() <= 2 ? 18 : 15);
        mark.setTypeface(Typeface.DEFAULT_BOLD);
        mark.setGravity(Gravity.CENTER);
        mark.setMinWidth(dp(48));
        ...
        if (!row.optString("week", "").isEmpty()) {
            addGradeChip(meta, "неделя: " + row.optString("week", ""));
        }

        addAttendanceStatusCard(card, row);

        TextView gradeLabel = new TextView(this);
        if (markText.equalsIgnoreCase("Н") || markText.equalsIgnoreCase("НН")) {
            gradeLabel.setText("Пропуск: " + markText.toUpperCase(ru));
        } else {
            gradeLabel.setText(markText.isEmpty()
                    ? "Оценка: нет"
                    : "Оценка: " + markText.toUpperCase(ru));
        }
    ...
    private void showGradeProblems(JSONArray rows) {
        ...
        TextView subtitle = new TextView(this);
        subtitle.setText("Здесь только проблемы из уже собранного списка оценок. Данные берутся из того же поля mark, как в v1.17.14.");
        subtitle.setTextColor(textMuted);
        subtitle.setTextSize(13);
        subtitle.setLineSpacing(dp(2), 1.0f);
        LinearLayout.LayoutParams subtitleParams = new LinearLayout.LayoutParams(
        ...
        for (int i = 0; i < rows.length(); i++) {
            try {
                JSONObject row = rows.getJSONObject(i);
                String mark = problemMarkFromRow(row).trim();
        ...
        if (count == 0) {
            ...
            TextView emptyTitle = new TextView(this);
            emptyTitle.setText("Проблем нет");
            emptyTitle.setTextColor(textMain);
            emptyTitle.setTextSize(17);
            emptyTitle.setTypeface(Typeface.DEFAULT_BOLD);
            empty.addView(emptyTitle);
            ...
            TextView emptyText = new TextView(this);
            emptyText.setText("Двоек, Н и НН в этом списке не найдено. Можно вернуться ко всем оценкам кнопкой выше.");
            emptyText.setTextColor(textMuted);
            emptyText.setTextSize(14);
            emptyText.setLineSpacing(dp(2), 1.0f);
            LinearLayout.LayoutParams emptyTextParams = new LinearLayout.LayoutParams(
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.19.11");
