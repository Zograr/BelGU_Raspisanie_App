...
public class MainActivity extends Activity {
    ...
    private int quickButtonBg(String title) {
        ...
        if (t.contains("оцен")) {
            return darkMode ? Color.rgb(38, 55, 47) : Color.rgb(232, 246, 237);
        }
        if (t.contains("преп")) {
            return darkMode ? Color.rgb(45, 50, 65) : Color.rgb(235, 240, 252);
        }
        if (t.contains("тем")) {
            return darkMode ? Color.rgb(55, 48, 64) : Color.rgb(245, 238, 252);
        }
        if (t.contains("обнов")) {
            return darkMode ? Color.rgb(58, 51, 40) : Color.rgb(250, 243, 229);
        }
    ...
    private int quickButtonStroke(String title) {
        ...
        if (t.contains("оцен")) {
            return darkMode ? Color.rgb(70, 96, 82) : Color.rgb(187, 223, 198);
        }
        if (t.contains("преп")) {
            return darkMode ? Color.rgb(73, 82, 112) : Color.rgb(194, 207, 238);
        }
        if (t.contains("тем")) {
            return darkMode ? Color.rgb(90, 75, 112) : Color.rgb(218, 198, 238);
        }
        if (t.contains("обнов")) {
            return darkMode ? Color.rgb(104, 86, 55) : Color.rgb(232, 210, 168);
        }
    ...
    private int quickButtonAccent(String title) {
        ...
        if (t.contains("оцен")) {
            return darkMode ? Color.rgb(176, 230, 194) : Color.rgb(42, 117, 67);
        }
        if (t.contains("преп")) {
            return darkMode ? Color.rgb(184, 204, 255) : Color.rgb(59, 87, 160);
        }
        if (t.contains("тем")) {
            return darkMode ? Color.rgb(218, 190, 255) : Color.rgb(122, 77, 160);
        }
        if (t.contains("обнов")) {
            return darkMode ? Color.rgb(247, 213, 151) : Color.rgb(155, 105, 28);
        }
    ...
    private void addGradesLoginForm() {
        ...
        note.setText("Пароль не сохраняется в приложении. Галочка «Не выходить» сохраняет сессию сайта через cookies WebView. Если сайт сбросит сессию, нужно будет войти заново.");
    ...
    private void loadNextSemesterWeek() {
        if (gradesSemesterWeekIndex >= gradesSemesterWeeks.length()) {
            ...
            addGradesMessage("Семестр подгружен",
                    "Собрано записей: " + gradesSemesterRows.length());
            return;
        }
    ...
    private void showGradeProblems(JSONArray rows) {
        ...
        if (count == 0) {
            addGradesMessage("Проблем нет", "Двоек, Н и НН в собранных оценках не найдено.");
        }
    }
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.18.8");
