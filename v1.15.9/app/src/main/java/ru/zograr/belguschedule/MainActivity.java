...
public class MainActivity extends Activity {
    ...
    private void refreshTeacherScheduleFromSite(TeacherItem teacher) {
        ...
        if (teachersPanelContent != null) {
            ...
            addTeacherMessage("Обновляю расписание преподавателя",
                    resolved.name + "\nЗагружаю полное расписание с сайта БелГУ...");
        }

        new Thread(() -> {
            try {
                String html = fetchUrl(TEACHERS_URL + "index.php?teacher=" + resolved.id);
                String cleaned = cleanupSection(stripHtml(html));
                main.post(() -> showTeacherScheduleText(resolved, cleaned));
            } catch (Exception e) {
                main.post(() -> {
                    if (teachersPanelContent != null) {
                        teachersPanelContent.removeAllViews();
                        addTeacherMessage("Не удалось обновить расписание",
                                e.getMessage() == null ? "Ошибка сети или сайт БелГУ не ответил." : e.getMessage());
                        addTeacherRefreshButton(resolved);
                    }
                });
            }
        }).start();
    ...
    private void addTeacherRefreshButton(TeacherItem teacher) {
        ...
        LinearLayout button = settingsButtonCard("Обновить расписание преподавателя",
                "Заново загрузить полное расписание с сайта БелГУ");
    ...
    private void showCachedTeacherSchedule(TeacherItem teacher) {
        ...
        addTeacherRefreshButton(teacher);
        ...
        if (total == 0) {
            addTeacherMessage("Пары не найдены",
                    "На сайте БелГУ для этого преподавателя на выбранной неделе пары не найдены. "
                            + "Можно нажать «Обновить расписание преподавателя» или проверить другую неделю на сайте.");
        }
    }
