...
public class MainActivity extends Activity {
    ...
    private static final String EASTER_EGG_URL = "https://s10.iimage.su/s/23/g4k0i8ax3w59qFLt24JWOmGIljsoUNRTo3BeCpItw.png";
    ...
    private TextView easterEggLoadingText;
    ...
    private List<TeacherItem> fetchTeacherList() throws Exception {
        ...
        // Самый надёжный вариант для твоего приложения: берём преподавателей прямо
        // из расписания выбранной группы. На странице группы у фамилий уже есть
        // ссылки вида teachers/index.php?teacher=ID, как на сайте БелГУ.
        try {
        ...
        // Запасной вариант: если ID преподавателя сайт не отдал, берём имена из уже
        // сохранённого расписания группы, чтобы поиск всё равно был полезным.
    ...
    private void addTeacherIfValid(List<TeacherItem> result, String idRaw, String nameRaw, boolean cachedTeacher) {
        String name = nameRaw == null ? "" : stripHtml(nameRaw).replaceAll("\\s+", " ").trim();
        name = name.replaceAll("\\s*i\\s*$", "").trim();

        if (name.length() < 5) return;
        String lower = name.toLowerCase(ru);
        if (!Pattern.compile("[А-Яа-яЁё]").matcher(name).find()) return;
    ...
    private void showCachedTeacherSchedule(TeacherItem teacher) {
        ...
        note.setText("Показаны пары из сохранённого расписания твоей группы. Полное расписание преподавателя сайт не отдал напрямую.");
        ...
        for (DaySchedule day : days) {
            ...
            for (String pair : pairs) {
                if (!pair.toLowerCase(ru).contains(teacher.name.toLowerCase(ru))) continue;
    ...
    private void showEasterEgg() {
        ...
        loadRemoteEasterEggImage();
        easterEggPanel.animate().alpha(1f).setDuration(180).start();
    }

    private void loadRemoteEasterEggImage() {
        if (easterEggImageView == null) return;

        easterEggImageView.setImageDrawable(null);
        easterEggImageView.setVisibility(View.INVISIBLE);
        if (easterEggLoadingText != null) {
            easterEggLoadingText.setText("Загружаю пасхалку...");
            easterEggLoadingText.setVisibility(View.VISIBLE);
        }

        new Thread(() -> {
            HttpURLConnection connection = null;
            try {
                URL url = new URL(EASTER_EGG_URL);
                    ...
                    if (bitmap != null) {
                        main.post(() -> {
                            if (easterEggImageView != null) {
                                easterEggImageView.setImageBitmap(bitmap);
                                easterEggImageView.setVisibility(View.VISIBLE);
                            }
                            if (easterEggLoadingText != null) {
                                easterEggLoadingText.setVisibility(View.GONE);
                            }
                        });
                        return;
                    }
                }
                main.post(() -> showEasterEggLoadFailed());
            } catch (Exception e) {
                main.post(() -> showEasterEggLoadFailed());
            } finally {
    ...
    private void showEasterEggLoadFailed() {
        if (easterEggLoadingText != null) {
            easterEggLoadingText.setText("Не удалось загрузить картинку по ссылке");
            easterEggLoadingText.setVisibility(View.VISIBLE);
        }
        if (easterEggImageView != null) {
            easterEggImageView.setVisibility(View.INVISIBLE);
    ...
    private void buildEasterEggPanel() {
        ...
        easterEggLoadingText = new TextView(this);
        easterEggLoadingText.setText("Загружаю пасхалку...");
        easterEggLoadingText.setTextColor(Color.argb(210, 255, 255, 255));
        easterEggLoadingText.setTextSize(16);
        easterEggLoadingText.setGravity(Gravity.CENTER);
        easterEggPanel.addView(easterEggLoadingText, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT));
