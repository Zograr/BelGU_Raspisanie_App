...
public class MainActivity extends Activity {
    ...
    private List<TeacherItem> fetchTeacherList() throws Exception {
        ...
        String[] sources = new String[] {
                ...
                "https://bsuedu.ru/bsu/education/schedule/",
                "https://bsuedu.ru/bsu/resource/schedule_old/"
        };
        ...
        if (result.isEmpty()) {
            throw new Exception("Не получилось получить список преподавателей с сайта.");
        }
        return result;
    }
    ...
    private void collectTeachersFromHtml(List<TeacherItem> result, String html) {
        ...
        collectTeachersByPattern(result, html,
                Pattern.compile("(?is)(?:\\\"id\\\"|\\\"value\\\")\\s*:\\s*\\\"?(\\d+)\\\"?[^\\{\\}]{0,220}?(?:\\\"name\\\"|\\\"label\\\"|\\\"text\\\")\\s*:\\s*\\\"([^\\\"]+)\\\""), 1, 2);
        collectTeachersByPattern(result, html,
                Pattern.compile("(?is)(?:teacher(?:Id)?|teacher_id)\\s*[=:]\\s*[\"\']?(\\d+)[\"\']?[^\\n\\r<>{}]{0,220}?(?:name|title|fio|label|text)\\s*[=:]\\s*[\"\']([^\"\']+)[\"\']"), 1, 2);
    }
    ...
    private void collectTeachersByPattern(List<TeacherItem> result, String html, Pattern pattern, int idGroup, int nameGroup) {
        ...
        while (matcher.find()) {
            ...
            addTeacherIfValid(result, id, name);
        }
    }
    ...
    private void addTeacherIfValid(List<TeacherItem> result, String idRaw, String nameRaw) {
        String id = idRaw == null ? "" : idRaw.replaceAll("[^0-9]", "").trim();
        String name = nameRaw == null ? "" : stripHtml(nameRaw).replaceAll("\\s+", " ").trim();

        if (id.isEmpty() || name.length() < 5) return;
        ...
        for (TeacherItem item : result) {
            if (item.id.equals(id) || item.name.equalsIgnoreCase(name)) {
                return;
            }
        }
