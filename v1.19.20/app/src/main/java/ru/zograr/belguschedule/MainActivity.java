...
public class MainActivity extends Activity {
    ...
    private static final String KEY_DEKANAT_REMEMBER_PASSWORD = "dekanat_remember_password";
    private static final String KEY_DEKANAT_PASSWORD_BLOB = "dekanat_password_blob";
    private static final String DEKANAT_KEYSTORE_ALIAS = "BelGUScheduleDekanatPasswordKey";
    private static final String KEY_DEKANAT_SESSION_TIME = "dekanat_session_time";
    private static final long DEKANAT_SESSION_MAX_MS = 12L * 60L * 60L * 1000L;
    private static final long DEKANAT_SESSION_REFRESH_MS = 10L * 60L * 60L * 1000L;
    ...
    private TextView gradesSelectedSemesterView;
    private TextView gradesSelectedWeekView;
    private TextView gradesResetSelectionButton;
    private boolean gradesResetSelectionButtonVisible = false;
    private TextView gradesOpenJournalButton;
    private boolean gradesJournalVisible = false;
    ...
    private boolean gradesJournalReadyMessageShown = false;
    ...
    private String gradesLoginNotice = "";
    private boolean gradesAutoLoginInProgress = false;
    private JSONArray gradesLastRows = new JSONArray();
    private JSONArray gradesProblemRows = new JSONArray();
    private JSONArray gradesProblemWeeks = new JSONArray();
    private int gradesProblemWeekIndex = 0;
    private int gradesProblemTotalWeeks = 0;
    ...
    private void refreshSettingsPanelContent() {
        ...
        addSettingsSection("Быстрый доступ");
        ...
        addSettingsSection("Автообновление");
    ...
    private void addSettingsQuickActions() {
        ...
        settingsQuickButton(row1, "Оценки", "журнал СПО", v -> showGradesPanel(), true);
        settingsQuickButton(row1, "Преподы", "расписание", v -> showTeachersPanel(), false);
        ...
        settingsQuickButton(row2, "Темы", themeDisplayName(themeMode), v -> showThemePanel(), true);
        settingsQuickButton(row2, "Обновить", "GitHub", v -> checkForUpdates(true), false);
    ...
    private void settingsQuickButton(LinearLayout row, String title, String subtitle, View.OnClickListener listener, boolean left) {
        ...
        startQuickButtonAppearAnimation(button, left ? 30 : 110);
        startQuickButtonShimmer(button, title);
    }
    ...
    private void startQuickButtonShimmer(LinearLayout button, String title) {
        ...
        animator.addUpdateListener(animation -> {
            ...
            int base = quickButtonBg(title);
            int accent = quickButtonAccent(title);

            int left = blendColor(base, accent, darkMode ? 0.10f + value * 0.18f : 0.16f + value * 0.22f);
            int center = blendColor(base, cardSurface, darkMode ? 0.03f + value * 0.05f : 0.05f + value * 0.08f);
            int right = blendColor(base, accent, darkMode ? 0.24f - value * 0.12f : 0.32f - value * 0.18f);

            GradientDrawable drawable = new GradientDrawable(
                    GradientDrawable.Orientation.LEFT_RIGHT,
                    new int[]{left, center, right});
            drawable.setCornerRadius(dp(16));
            drawable.setStroke(dp(1), blendColor(quickButtonStroke(title), accent, 0.25f + value * 0.25f));

            button.setBackground(drawable);
            button.setAlpha(0.94f + value * 0.06f);
        });
    ...
    private void startQuickButtonAppearAnimation(View view, int delayMs) {
        view.setAlpha(0f);
    ...
    private int quickButtonIndex(String title) {
        ...
        if (t.contains("оцен")) return 0;
        if (t.contains("преп")) return 1;
        if (t.contains("тем")) return 2;
        if (t.contains("обнов")) return 3;
    ...
    private LinearLayout settingsHeroCard() {
        ...
        card.setBackground(settingsHeroAnimatedBackground(0f));
        card.setClickable(false);
        card.setFocusable(false);
        startSettingsHeroAnimation(card);
        return card;
    }



    private void startSettingsHeroAnimation(LinearLayout card) {
        ValueAnimator animator = ValueAnimator.ofFloat(0f, 1f);
        animator.setDuration(4200);
        animator.setRepeatMode(ValueAnimator.REVERSE);
        animator.setRepeatCount(ValueAnimator.INFINITE);
        animator.addUpdateListener(animation -> {
            if (!settingsOpen || card.getParent() == null) {
                animation.cancel();
                return;
            }

            float value = (float) animation.getAnimatedValue();
            card.setBackground(settingsHeroAnimatedBackground(value));
        });
    ...
    private GradientDrawable settingsHeroAnimatedBackground(float progress) {
        int start = blendColor(headerStart, green, 0.10f + progress * 0.10f);
        int end = blendColor(headerEnd, greenDark, 0.08f + progress * 0.12f);
    ...
    private int blendColor(int first, int second, float amount) {
        float a = Math.max(0f, Math.min(1f, amount));

        int r = Math.round(Color.red(first) * (1f - a) + Color.red(second) * a);
        int g = Math.round(Color.green(first) * (1f - a) + Color.green(second) * a);
        int b = Math.round(Color.blue(first) * (1f - a) + Color.blue(second) * a);
    ...
    private LinearLayout settingsCard() {
        ...
        startCardAppearAnimation(card);
        return card;
    }


    private void startCardAppearAnimation(View view) {
        view.setAlpha(0f);
    ...
    private LinearLayout gradesHintHeader(String titleText, String badgeText) {
        LinearLayout header = new LinearLayout(this);
        ...
        marker.setBackground(pillBackground(green, green));
        LinearLayout.LayoutParams markerParams = new LinearLayout.LayoutParams(dp(28), dp(28));
        markerParams.setMargins(0, 0, dp(10), 0);
        header.addView(marker, markerParams);
        startSoftBreathingAnimation(marker);
        ...
        badge.setTextColor(greenDark);
        badge.setTextSize(11);
        badge.setTypeface(Typeface.DEFAULT_BOLD);
        badge.setGravity(Gravity.CENTER);
        badge.setPadding(dp(9), dp(5), dp(9), dp(5));
        badge.setBackground(pillBackground(greenSoft, border));
    ...
    private void addDismissibleGradesHint(LinearLayout card, String preferenceKey) {
        TextView hide = gradesSmallButton("Скрыть подсказку");
        hide.setTextSize(12);
        ...
        card.addView(hide, params);
    }



    private String gradeCode(String value) {
        if (value == null) return "";

        String code = value.trim().toUpperCase(ru);

        if (code.equals("П")) return "П";
        if (code.equals("О")) return "О";
        if (code.equals("Н")) return "Н";
        if (code.equals("НН")) return "НН";

        return "";
    }

    private String gradeCodeLabel(String code) {
        if (code == null) return "";

        String c = code.trim().toUpperCase(ru);

        if (c.equals("П")) return "Присутствовал";
        if (c.equals("О")) return "Онлайн";
        if (c.equals("Н")) return "Пропуск";
        if (c.equals("НН")) return "Двойной пропуск";

        return "";
    }

    private int gradeCodeColor(String code) {
        if (code == null) return green;

        String c = code.trim().toUpperCase(ru);

        if (c.equals("П")) return green;
        if (c.equals("О")) return Color.rgb(74, 105, 170);
        if (c.equals("Н") || c.equals("НН")) return Color.rgb(198, 80, 72);

        return green;
    }

    private void addGradeLetterBadge(LinearLayout parent, String value) {
        String code = gradeCode(value);
        if (code.isEmpty()) return;

        String label = gradeCodeLabel(code);

        LinearLayout badge = new LinearLayout(this);
        badge.setOrientation(LinearLayout.HORIZONTAL);
        badge.setGravity(Gravity.CENTER_VERTICAL);
        badge.setPadding(dp(7), dp(5), dp(10), dp(5));
        badge.setBackground(pillBackground(greenSoft, border));
        ...
        letter.setMinWidth(dp(code.length() > 1 ? 30 : 25));
        letter.setPadding(dp(7), dp(4), dp(7), dp(4));
        letter.setBackground(pillBackground(gradeCodeColor(code), gradeCodeColor(code)));
        badge.addView(letter);

        TextView labelView = new TextView(this);
        labelView.setText(label);
        labelView.setTextColor(textMain);
        ...
        badge.addView(labelView, labelParams);
    ...
    private void applyThemeCheckBoxTint(CheckBox checkBox) {
        int[][] states = new int[][]{
    ...
    private void startSoftBreathingAnimation(View view) {
        if (view == null) return;
            ...
            view.animate()
                    ...
                    .withEndAction(() -> {
                        ...
                        view.animate()
                                ...
                                .alpha(1.0f)
                                .setDuration(1150)
                                .withEndAction(() -> startSoftBreathingAnimation(view))
                                .start();
                    })
                    .start();
        }, 350);
    ...
    private void addGradesJournalStatusCard() {
        LinearLayout statusCard = settingsCard();
        statusCard.setOrientation(LinearLayout.VERTICAL);
        statusCard.setPadding(dp(15), dp(14), dp(15), dp(14));
        statusCard.setBackground(locationBackground(gradesJournalReady ? greenSoft : cardSurface, gradesJournalReady ? green : border));
        ...
        TextView dot = new TextView(this);
        dot.setText(gradesJournalReady ? "✓" : "!");
        dot.setTextColor(Color.WHITE);
        dot.setTextSize(14);
        dot.setTypeface(Typeface.DEFAULT_BOLD);
        dot.setGravity(Gravity.CENTER);
        dot.setBackground(pillBackground(gradesJournalReady ? green : Color.rgb(198, 80, 72),
                gradesJournalReady ? green : Color.rgb(198, 80, 72)));
        LinearLayout.LayoutParams dotParams = new LinearLayout.LayoutParams(dp(30), dp(30));
        dotParams.setMargins(0, 0, dp(10), 0);
        row.addView(dot, dotParams);
        startSoftBreathingAnimation(dot);

        TextView title = new TextView(this);
        title.setText(gradesJournalReady ? "Журнал открыт" : "Журнал не готов");
        title.setTextColor(textMain);
        title.setTextSize(18);
        title.setTypeface(Typeface.DEFAULT_BOLD);
        ...
        TextView badge = new TextView(this);
        badge.setText(gradesJournalReady ? "готов" : "ожидание");
        badge.setTextColor(gradesJournalReady ? greenDark : textMuted);
        badge.setTextSize(11);
        badge.setTypeface(Typeface.DEFAULT_BOLD);
        badge.setGravity(Gravity.CENTER);
        badge.setPadding(dp(9), dp(5), dp(9), dp(5));
        badge.setBackground(pillBackground(gradesJournalReady ? greenSoft : cardSurface, border));
        row.addView(badge);

        TextView subtitle = new TextView(this);
        subtitle.setText(gradesJournalReady
                ? "Можно выбирать семестр, неделю и собирать оценки."
                : "Если сайт вернул страницу входа, нажми «Войти заново» или «Войти сохранённым паролем».");
        subtitle.setTextColor(textMuted);
        ...
        statusCard.addView(subtitle, subtitleParams);

        addGradesCard(statusCard);
    }

    private void addGradesLoginForm() {
        SharedPreferences prefs = getSharedPreferences(PREFS, MODE_PRIVATE);

        if (!gradesLoginNotice.trim().isEmpty()) {
            LinearLayout notice = settingsCard();
            notice.setOrientation(LinearLayout.VERTICAL);
            notice.setPadding(dp(14), dp(13), dp(14), dp(13));

            notice.addView(gradesHintHeader("Нужно войти заново", "сессия"));

            TextView noticeText = new TextView(this);
            noticeText.setText(gradesLoginNotice);
            noticeText.setTextColor(textMuted);
            noticeText.setTextSize(13);
            noticeText.setLineSpacing(dp(2), 1.0f);
            LinearLayout.LayoutParams noticeTextParams = new LinearLayout.LayoutParams(
                    LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
            noticeTextParams.setMargins(0, dp(8), 0, 0);
            notice.addView(noticeText, noticeTextParams);

            addGradesCard(notice);
        }

        if (!prefs.getBoolean("grades_login_hint_hidden", false)) {
            LinearLayout info = settingsCard();
            info.setOrientation(LinearLayout.VERTICAL);
            info.setPadding(dp(14), dp(13), dp(14), dp(13));

            info.addView(gradesHintHeader("Вход в личный кабинет", "подсказка"));

            TextView note = new TextView(this);
            note.setText("Пароль можно сохранить на устройстве: он хранится не открытым текстом, а в зашифрованном виде через Android Keystore.");
            ...
            addDismissibleGradesHint(info, "grades_login_hint_hidden");
            addGradesCard(info);
        }
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            loginInput.setAutofillHints(View.AUTOFILL_HINT_USERNAME);
            loginInput.setImportantForAutofill(View.IMPORTANT_FOR_AUTOFILL_YES);
        }
        addGradesView(loginInput, 8);

        EditText passwordInput = gradesInput("Пароль", true);
        String savedPassword = loadDekanatSavedPassword();
        if (!savedPassword.isEmpty()) {
            passwordInput.setText(savedPassword);
        }
        if (Build.VERSION.SDK_INT >= 26) {
            passwordInput.setAutofillHints(View.AUTOFILL_HINT_PASSWORD);
            passwordInput.setImportantForAutofill(View.IMPORTANT_FOR_AUTOFILL_YES);
        }
        addGradesView(passwordInput, 8);

        CheckBox rememberPasswordCheck = new CheckBox(this);
        rememberPasswordCheck.setText("Запомнить пароль на этом устройстве");
        rememberPasswordCheck.setTextColor(textMain);
        rememberPasswordCheck.setTextSize(14);
        rememberPasswordCheck.setChecked(prefs.getBoolean(KEY_DEKANAT_REMEMBER_PASSWORD, false) && !savedPassword.isEmpty());
        rememberPasswordCheck.setPadding(dp(4), 0, dp(4), dp(6));
        applyThemeCheckBoxTint(rememberPasswordCheck);
        addGradesView(rememberPasswordCheck, 8);

        TextView loginButton = gradesBigButton(savedPassword.isEmpty()
                ? "Войти и открыть журнал СПО"
                : "Войти сохранённым паролем");
        loginButton.setOnClickListener(v -> {
            ...
            getSharedPreferences(PREFS, MODE_PRIVATE)
                    ...
                    .putBoolean(KEY_DEKANAT_REMEMBER_PASSWORD, rememberPasswordCheck.isChecked())
                    .apply();

            if (rememberPasswordCheck.isChecked()) {
                saveDekanatPassword(password);
            } else {
                clearDekanatSavedPassword();
            }
        ...
        manualButton.setOnClickListener(v -> {
            gradesLoginNotice = "";
            gradesAutoLoginInProgress = false;
            pendingDekanatLogin = "";
            pendingDekanatPassword = "";
            gradesJournalReady = false;
            gradesJournalReadyMessageShown = false;
            ensureGradesWebView();
            attachGradesWebView(dp(520));
            gradesWebView.loadUrl(DEKANAT_LOGIN_URL);
            addGradesMessage("Сайт открыт вручную",
                    "Это обычное окно сайта. Введи данные прямо на странице. Автовход и сохранённый пароль здесь не используются.");
        });
    ...
    private void saveDekanatPassword(String password) {
        ...
        try {
            ...
            cipher.init(Cipher.ENCRYPT_MODE, getDekanatSecretKey());
            ...
            getSharedPreferences(PREFS, MODE_PRIVATE)
                    .edit()
                    .putString(KEY_DEKANAT_PASSWORD_BLOB, payload)
                    .putBoolean(KEY_DEKANAT_REMEMBER_PASSWORD, true)
                    .apply();
        } catch (Exception e) {
            Toast.makeText(this, "Не удалось сохранить пароль защищённо", Toast.LENGTH_LONG).show();
            clearDekanatSavedPassword();
        }
    }

    private String loadDekanatSavedPassword() {
        try {
            String payload = getSharedPreferences(PREFS, MODE_PRIVATE).getString(KEY_DEKANAT_PASSWORD_BLOB, "");
            if (payload == null || payload.trim().isEmpty() || !payload.contains(".")) return "";
            ...
            Cipher cipher = Cipher.getInstance("AES/GCM/NoPadding");
            cipher.init(Cipher.DECRYPT_MODE, getDekanatSecretKey(), new GCMParameterSpec(128, iv));
    ...
    private void clearDekanatSavedPassword() {
        getSharedPreferences(PREFS, MODE_PRIVATE)
                .edit()
                .remove(KEY_DEKANAT_PASSWORD_BLOB)
                .putBoolean(KEY_DEKANAT_REMEMBER_PASSWORD, false)
                .apply();
    }

    private SecretKey getDekanatSecretKey() throws Exception {
        KeyStore keyStore = KeyStore.getInstance("AndroidKeyStore");
        keyStore.load(null);

        if (!keyStore.containsAlias(DEKANAT_KEYSTORE_ALIAS)) {
            KeyGenerator keyGenerator = KeyGenerator.getInstance(KeyProperties.KEY_ALGORITHM_AES, "AndroidKeyStore");
            KeyGenParameterSpec spec = new KeyGenParameterSpec.Builder(
                    DEKANAT_KEYSTORE_ALIAS,
        ...
        KeyStore.Entry entry = keyStore.getEntry(DEKANAT_KEYSTORE_ALIAS, null);
        if (!(entry instanceof KeyStore.SecretKeyEntry)) {
            throw new Exception("Keystore key is not SecretKeyEntry");
        }
    ...
    private void addGradesTools() {
        ...
        addGradesJournalStatusCard();

        if (!prefs.getBoolean("grades_tools_hint_hidden", false)) {
            LinearLayout warning = settingsCard();
            warning.setOrientation(LinearLayout.VERTICAL);
            warning.setPadding(dp(14), dp(13), dp(14), dp(13));

            warning.addView(gradesHintHeader("Журнал СПО", "инструкция"));

            TextView sub = new TextView(this);
            sub.setText("Для обычного просмотра выбери семестр и неделю. После сбора оценок можно отдельно открыть список двоек и Н/НН — без повторного кривого парсинга.");
            sub.setTextColor(textMuted);
            sub.setTextSize(13);
            ...
            addDismissibleGradesHint(warning, "grades_tools_hint_hidden");
            addGradesCard(warning);
        }

        addGradesSelectionStatus();
        ...
        actionsHint.setText("Сначала выбери семестр и неделю, затем собери оценки. После этого можно открыть отдельный список двоек и пропусков.");
        ...
        TextView collect = gradesBigButton("Собрать оценки");
        ...
        gradesOpenJournalButton = gradesSmallButton(gradesJournalVisible ? "Закрыть журнал" : "Открыть журнал");
        gradesOpenJournalButton.setOnClickListener(v -> toggleGradesJournal(gradesOpenJournalButton));
        LinearLayout.LayoutParams openParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        row2.addView(gradesOpenJournalButton, openParams);
        ...
        attachGradesWebView(gradesJournalVisible ? dp(520) : dp(1));
    ...
    private TextView gradesBigButton(String text) {
        ...
        startSoftBreathingAnimation(button);
        return button;
    }
        ...
        gradesWebView.setWebViewClient(new WebViewClient() {
            ...
            public void onPageFinished(WebView view, String url) {
                ...
                if (fixedUrl.contains("journal_spo")) {
                    boolean wasAutoLogin = gradesAutoLoginInProgress;
                    ...
                    getSharedPreferences(PREFS, MODE_PRIVATE)
                            .edit()
                            .putLong(KEY_DEKANAT_SESSION_TIME, System.currentTimeMillis())
                            .apply();

                    if (wasAutoLogin && gradesPanelOpen) {
                        main.post(() -> {
                            refreshGradesPanelContent();
                            addGradesMessageNoAnimation("Журнал открыт", "Теперь можно выбрать семестр и неделю.");
                        });
                    } else if (!gradesJournalReadyMessageShown) {
                        gradesJournalReadyMessageShown = true;
                        addGradesMessageNoAnimation("Журнал открыт", "Теперь можно выбрать семестр и неделю.");
                    }
                } else {
                    boolean looksLikeLoginPage = fixedUrl.contains("login") || fixedUrl.contains("portfolio");

                    if (looksLikeLoginPage && gradesAutoLoginInProgress) {
                        return;
                    }

                    if (looksLikeLoginPage && pendingDekanatLogin.isEmpty() && gradesLoggedIn && !gradesJournalReady) {
                        gradesAutoLoginInProgress = false;
                        gradesLoggedIn = false;
                        gradesLoginNotice = "Сессия сайта истекла. Нажми «Войти сохранённым паролем» или введи пароль заново, чтобы снова открыть журнал.";
                        main.post(() -> refreshGradesPanelContent());
                    } else if (!gradesJournalReady) {
                        gradesJournalReadyMessageShown = false;
                    }
                }
            }
    ...
    private void startDekanatLogin(String login, String password) {
        ensureGradesWebView();
        gradesLoginNotice = "";
        gradesAutoLoginInProgress = true;
        ...
        gradesJournalReady = false;
        gradesJournalReadyMessageShown = false;
        addGradesMessage("Вхожу в личный кабинет", "Журнал загрузится скрыто внутри приложения...");
        gradesWebView.loadUrl(DEKANAT_LOGIN_URL);

        main.postDelayed(() -> {
            if (gradesAutoLoginInProgress && !gradesJournalReady) {
                gradesAutoLoginInProgress = false;
                gradesLoggedIn = false;
                gradesLoginNotice = "Вход не завершился автоматически. Нажми «Войти сохранённым паролем» ещё раз или открой сайт вручную.";
                refreshGradesPanelContent();
            }
        }, 22000);
    }
    ...
    private void injectDekanatLogin() {
        ...
        gradesWebView.evaluateJavascript(js, value -> main.postDelayed(() -> {
            gradesJournalReady = false;
            addGradesMessage("Открываю журнал СПО", "Загрузка идёт скрыто. Дождись сообщения «Журнал готов».");
            gradesWebView.loadUrl(DEKANAT_JOURNAL_URL);
        }, 1600));
    }
            ...
            try {
                ...
                if (options.length() == 0) {
                    addGradesMessage("Список не найден", semester
                            ...
                            : "Не нашёл список недель с датами. Сначала выбери семестр, подожди пару секунд и попробуй ещё раз.");
                    return;
                }
    ...
    private String buildGetSemesterOptionsScript() {
        return "(function(){" +
                ...
                "function isLangText(t){t=txt(t).toLowerCase();return t==='ru'||t==='rus'||t==='рус'||t==='en'||t==='eng'||t==='english'||t.indexOf('русский')>=0||t.indexOf('англ')>=0;}" +
                "function isLanguageSelect(s){var os=[].slice.call(s.options).map(txt).filter(Boolean);return os.length>0&&os.every(isLangText);}" +
                "function isSemesterSelect(s){if(isLanguageSelect(s))return false;var os=[].slice.call(s.options).map(txt).filter(Boolean);var nums=os.filter(function(t){return /^[1-8]$/.test(t);});var bad=os.filter(function(t){return /\\d{1,2}[.\\/]\\d{1,2}/.test(t)||isLangText(t);});return nums.length>=2&&bad.length===0;}" +
                "var selects=[].slice.call(document.querySelectorAll('select'));" +
                "var target=selects.find(isSemesterSelect);" +
                "if(!target)return JSON.stringify([]);" +
                "return JSON.stringify([].slice.call(target.options).map(function(o,i){return {index:i,text:txt(o)||('Семестр '+(i+1))};}).filter(function(x){return x.text&&x.text.toLowerCase().indexOf('выберите')<0&&/^[1-8]$/.test(x.text);}));" +
                "})()";
    }
    private String buildGetWeekOptionsScript() {
        return "(function(){" +
                ...
                "function isLangText(t){t=txt(t).toLowerCase();return t==='ru'||t==='rus'||t==='рус'||t==='en'||t==='eng'||t==='english'||t.indexOf('русский')>=0||t.indexOf('англ')>=0;}" +
                "function isLanguageSelect(s){var os=[].slice.call(s.options).map(txt).filter(Boolean);return os.length>0&&os.every(isLangText);}" +
                "function hasDate(t){t=txt(t);return /\\d{1,2}[.\\/]\\d{1,2}([.\\/]\\d{2,4})?/.test(t);}" +
                "function hasWeekWord(t){t=txt(t).toLowerCase();return t.indexOf('нед')>=0||t.indexOf('week')>=0;}" +
                "function isSemesterSelect(s){if(isLanguageSelect(s))return false;var os=[].slice.call(s.options).map(txt).filter(Boolean);if(os.length<2)return false;var nums=os.filter(function(t){return /^[1-8]$/.test(t);});var date=os.filter(hasDate);return nums.length>=2&&date.length===0;}" +
                "function weekScore(s){if(isLanguageSelect(s)||isSemesterSelect(s))return -1000;var os=[].slice.call(s.options).map(txt).filter(Boolean);var score=0;os.forEach(function(t){if(isLangText(t))score-=50;if(hasDate(t))score+=5;if(hasWeekWord(t))score+=3;if(/[-–—]/.test(t))score+=1;});return score;}" +
                "var selects=[].slice.call(document.querySelectorAll('select'));" +
                "var best=null,bestScore=-999;" +
                "selects.forEach(function(s){var sc=weekScore(s);if(sc>bestScore){bestScore=sc;best=s;}});" +
                "if(!best||bestScore<=0)return JSON.stringify([]);" +
                "return JSON.stringify([].slice.call(best.options).map(function(o,i){return {index:i,text:txt(o)||('Неделя '+(i+1))};}).filter(function(x){var low=x.text.toLowerCase();return x.text&&low.indexOf('выберите')<0&&!/^[1-8]$/.test(x.text)&&!isLangText(x.text);}));" +
                "})()";
    }
    private void setGradesOption(boolean semester, int index, String label) {
        String js = "(function(){" +
                ...
                "var target=null;" +
                (semester
                        ? "target=selects.find(function(s){return isSemesterSelect(s);});"
                        : "var best=null,bestScore=-999;selects.forEach(function(s){var sc=weekScore(s);if(sc>bestScore){bestScore=sc;best=s;}});target=bestScore>0?best:null;") +
        ...
        gradesWebView.evaluateJavascript(js, value -> {
            refreshGradesSelectionViews();
            addGradesMessage(semester ? "Семестр выбран" : "Неделя выбрана",
                    label + "\nТеперь выбранное видно в блоке «Выбрано сейчас». Подожди загрузку таблицы и нажми «Собрать оценки».");
        });
    }


    private void collectAllGradeProblems() {
        ensureGradesWebView();

        gradesProblemRows = new JSONArray();
        gradesProblemWeeks = new JSONArray();
        gradesProblemWeekIndex = 0;
        gradesProblemTotalWeeks = 0;

        addGradesMessage("Собираю проблемы", "Ищу все недели выбранного семестра и собираю двойки, Н и НН...");
        ...
        gradesWebView.evaluateJavascript(buildGetWeekOptionsScript(), value -> {
            try {
                ...
                if (options.length() == 0) {
                    addGradesMessage("Недели не найдены",
                            "Сначала выбери семестр. Если семестр уже выбран — подожди пару секунд и нажми кнопку ещё раз.");
                    return;
                }

                gradesProblemWeeks = options;
                gradesProblemTotalWeeks = options.length();
                gradesProblemWeekIndex = 0;
                collectNextProblemWeek();
            } catch (Exception e) {
                addGradesMessage("Ошибка сбора проблем",
                        e.getMessage() == null ? "Не удалось получить список недель." : e.getMessage());
            }
        });
    }

    private void collectNextProblemWeek() {
        if (gradesProblemWeekIndex >= gradesProblemWeeks.length()) {
            showAllGradeProblemsResult();
            return;
        }

        try {
            JSONObject week = gradesProblemWeeks.getJSONObject(gradesProblemWeekIndex);
            int index = week.optInt("index", gradesProblemWeekIndex);
            String label = week.optString("text", "Неделя " + (gradesProblemWeekIndex + 1));

            addGradesMessage("Собираю проблемы",
                    "Неделя " + (gradesProblemWeekIndex + 1) + " из " + gradesProblemTotalWeeks
                            + "\n" + label);

            setWeekByIndexForProblems(index, label);
        } catch (Exception e) {
            gradesProblemWeekIndex++;
            collectNextProblemWeek();
        }
    }

    private void setWeekByIndexForProblems(int index, String label) {
        String js = "(function(){" +
                "function txt(o){return ((o.innerText||o.textContent||o.value||'')+'').replace(/\\s+/g,' ').trim();}" +
                "function hasDate(t){t=txt(t);return /\\d{1,2}[.\\/]\\d{1,2}([.\\/]\\d{2,4})?/.test(t);}" +
                "function hasWeekWord(t){t=txt(t).toLowerCase();return t.indexOf('нед')>=0||t.indexOf('week')>=0;}" +
                "function isSemesterSelect(s){var os=[].slice.call(s.options).map(txt).filter(Boolean);if(os.length<2)return false;var nums=os.filter(function(t){return /^\\d{1,2}$/.test(t);});var date=os.filter(hasDate);return nums.length>=2&&date.length===0;}" +
                "function weekScore(s){var os=[].slice.call(s.options).map(txt).filter(Boolean);var score=0;os.forEach(function(t){if(hasDate(t))score+=5;if(hasWeekWord(t))score+=3;if(/[-–—]/.test(t))score+=1;});if(isSemesterSelect(s))score-=100;return score;}" +
                "var selects=[].slice.call(document.querySelectorAll('select'));" +
                "var best=null,bestScore=-999;selects.forEach(function(s){var sc=weekScore(s);if(sc>bestScore){bestScore=sc;best=s;}});" +
                "if(!best||bestScore<=0)return JSON.stringify({ok:false});" +
            ...
            gradesWebView.evaluateJavascript(buildCollectGradesScript(), rowsValue -> {
                try {
                    ...
                    for (int i = 0; i < rows.length(); i++) {
                        ...
                        if (isProblemGrade(row.optString("mark", ""))) {
                            row.put("week", label);
                            gradesProblemRows.put(row);
                        }
                    }
                } catch (Exception ignored) {
                }
    ...
    private void addCurrentWeekToSemesterManual() {
        ensureGradesWebView();

        if (!gradesJournalReady) {
            addGradesMessage("Журнал ещё загружается",
                    "Подожди сообщение «Журнал готов», потом выбери семестр и неделю.");
            return;
        }
        ...
        if (selectedGradesSemester == null || selectedGradesSemester.trim().isEmpty()
                ...
                || selectedGradesWeek.equalsIgnoreCase("весь семестр")) {
            addGradesMessage("Выбери неделю",
                    "Сначала выбери семестр и конкретную неделю. Когда оценки этой недели появились, нажми «Добавить неделю в семестр».");
            return;
        }

        addGradesMessage("Добавляю неделю",
                selectedGradesWeek + "\nЖду, пока оценки полностью прогрузятся...");
        collectCurrentWeekIntoManualSemester(0);
    }

    private void collectCurrentWeekIntoManualSemester(int attempt) {
        final String weekLabel = selectedGradesWeek == null ? "" : selectedGradesWeek;
        ...
        gradesWebView.evaluateJavascript(buildCollectGradesScript(), rowsValue -> {
            try {
                ...
                if (shouldRetryGradesCollect(rows) && attempt < 6) {
                    main.postDelayed(() -> collectCurrentWeekIntoManualSemester(attempt + 1), 1200);
                    return;
                }

                if (rows.length() == 0) {
                    addGradesMessage("Неделя не добавлена",
                            "Не удалось прочитать строки текущей недели. Попробуй нажать ещё раз после полной загрузки.");
                    return;
                }

                gradesSemesterRows = removeRowsForWeek(gradesSemesterRows, weekLabel);

                for (int i = 0; i < rows.length(); i++) {
                    JSONObject row = rows.getJSONObject(i);
                    row.put("week", weekLabel);
                    gradesSemesterRows.put(row);
                }

                selectedGradesWeek = "собрано вручную";
                showGradesRows(gradesSemesterRows);
                Toast.makeText(this,
                        "Неделя добавлена. Строк: " + gradesSemesterRows.length(),
                        Toast.LENGTH_LONG).show();
            } catch (Exception e) {
                addGradesMessage("Ошибка добавления недели",
                        e.getMessage() == null ? "Не удалось разобрать оценки." : e.getMessage());
            }
        });
    }

    private JSONArray removeRowsForWeek(JSONArray source, String weekLabel) {
        ...
        for (int i = 0; i < source.length(); i++) {
            try {
                ...
                String existingWeek = row.optString("week", "");
                if (!existingWeek.equals(weekLabel)) {
                    result.put(row);
                }
            } catch (Exception ignored) {
            }
    ...
    private void showManualSemesterRows() {
        if (gradesSemesterRows == null || gradesSemesterRows.length() == 0) {
            addGradesMessage("Семестр ещё не собран",
                    "Выбери неделю, дождись оценок и нажми «Добавить неделю в семестр». Повтори для нужных недель.");
            return;
        }
    ...
    private void clearManualSemesterRows() {
        gradesSemesterRows = new JSONArray();
        gradesSemesterWeeks = new JSONArray();
        gradesSemesterWeekIndex = 0;
        addGradesMessage("Собранный семестр очищен",
                "Теперь можно заново добавлять недели вручную.");
    }
    ...
    private void loadGradesForSelectedSemester() {
        ...
        if (!gradesJournalReady) {
            addGradesMessage("Журнал ещё загружается",
                    "Подожди сообщение «Журнал готов», потом нажми «Подгрузить семестр».");
            return;
        }

        if (selectedGradesSemester == null || selectedGradesSemester.trim().isEmpty()) {
            addGradesMessage("Выбери семестр",
                    "Сначала выбери семестр. После этого можно подгрузить все недели семестра.");
            return;
        }

        addGradesMessage("Подгружаю семестр", "Жду, пока сайт полностью обновит список недель...");
        main.postDelayed(() -> loadGradesForSelectedSemesterRetry(0), 1800);
    }

    private void loadGradesForSelectedSemesterRetry(int attempt) {
        gradesWebView.evaluateJavascript(buildGetWeekOptionsScript(), value -> {
            try {
                String jsonString = new JSONArray("[" + value + "]").getString(0);
                JSONArray weeks = new JSONArray(jsonString);

                if (weeks.length() == 0 && attempt < 5) {
                    main.postDelayed(() -> loadGradesForSelectedSemesterRetry(attempt + 1), 1200);
                    return;
                }

                if (weeks.length() == 0) {
                    addGradesMessage("Недели не найдены",
                            "Сайт не отдал список недель. Выбери семестр ещё раз и нажми «Подгрузить семестр».");
                    return;
                }
                ...
                addGradesMessage("Ошибка семестра",
                        e.getMessage() == null ? "Не удалось получить список недель." : e.getMessage());
            }
        });
    }
    ...
    private void loadNextSemesterWeek() {
        if (gradesSemesterWeekIndex >= gradesSemesterWeeks.length()) {
            selectedGradesWeek = "весь семестр";
            JSONArray finalRows = gradesSemesterRows;

            showGradesRows(finalRows);
            Toast.makeText(this, "Семестр подгружен. Записей: " + finalRows.length(), Toast.LENGTH_LONG).show();

            main.postDelayed(() -> showGradesRows(finalRows), 450);
            main.postDelayed(() -> showGradesRows(finalRows), 1200);
            return;
        }
        ...
        try {
            ...
            String label = week.optString("text", "Неделя " + (gradesSemesterWeekIndex + 1));

            addGradesMessage("Подгружаю семестр",
                    "Неделя " + (gradesSemesterWeekIndex + 1) + " из " + gradesSemesterWeeks.length()
                            + "\n" + label);
    ...
    private void selectWeekForSemesterLoad(int index, String label) {
        String js = "(function(){" +
                ...
                "function isLang(t){t=txt(t).toLowerCase();return t==='ru'||t==='eng'||t==='en'||t.indexOf('рус')>=0||t.indexOf('english')>=0;}" +
                ...
                "function weekScore(s){if(isSemSelect(s))return -100;var os=[].slice.call(s.options).map(txt).filter(Boolean);var score=0;os.forEach(function(t){if(hasDate(t))score+=5;if(t.toLowerCase().indexOf('нед')>=0)score+=2;if(/[-–—]/.test(t))score+=1;if(isLang(t))score-=20;});return score;}" +
                "var selects=[].slice.call(document.querySelectorAll('select'));" +
        ...
        selectedGradesWeek = label;
        gradesWebView.evaluateJavascript(js, value -> main.postDelayed(() -> collectSemesterWeekRows(label, 0), 5500));
    }



    private void collectSemesterWeekRows(String label, int attempt) {
        gradesWebView.evaluateJavascript(buildCollectGradesScript(), rowsValue -> {
            try {
                String jsonString = new JSONArray("[" + rowsValue + "]").getString(0);
                JSONArray rows = new JSONArray(jsonString);

                if (shouldRetrySemesterWeekCollect(rows) && attempt < 10) {
                    main.postDelayed(() -> collectSemesterWeekRows(label, attempt + 1), 1500);
                    return;
                }
    ...
    private boolean shouldRetrySemesterWeekCollect(JSONArray rows) {
        ...
        for (int i = 0; i < rows.length(); i++) {
            try {
                JSONObject row = rows.getJSONObject(i);
                if (!row.optString("subject", "").trim().isEmpty()) subjects++;

                String mark = problemMarkFromRow(row).trim().toLowerCase(ru);
                if (!mark.isEmpty()
                        && !mark.equals("п")
                        && !mark.equals("о")) {
                    realMarks++;
    ...
    private void collectGradesFromJournalRetry(int attempt) {
        gradesWebView.evaluateJavascript(buildCollectGradesScript(), value -> {
            try {
                String jsonString = new JSONArray("[" + value + "]").getString(0);
                JSONArray rows = new JSONArray(jsonString);

                if (shouldRetryGradesCollect(rows) && attempt < 5) {
                    main.postDelayed(() -> collectGradesFromJournalRetry(attempt + 1), 1200);
                    return;
                }
    ...
    private boolean shouldRetryGradesCollect(JSONArray rows) {
        if (rows == null || rows.length() == 0) return true;
        ...
        for (int i = 0; i < rows.length(); i++) {
            try {
                JSONObject row = rows.getJSONObject(i);
                if (!row.optString("subject", "").trim().isEmpty()) withSubject++;
                if (!problemMarkFromRow(row).trim().isEmpty()) marked++;
            } catch (Exception ignored) {
            }
        }

        // Если предметы уже есть, но оценок нет совсем, значит сайт ещё не успел дорисовать журнал.
        return withSubject > 0 && marked == 0;
    }
    ...
    private String buildCollectGradesScript() {
        return "(function(){" +
                ...
                "function dayName(s){s=clean(s).toLowerCase();var days=['понедельник','вторник','среда','четверг','пятница','суббота','воскресенье'];for(var i=0;i<days.length;i++){if(s.indexOf(days[i])>=0)return days[i].charAt(0).toUpperCase()+days[i].slice(1);}return '';}" +
                ...
                "function isType(s){s=clean(s).toLowerCase();return /^(п|о|л|лек|лек\\.|лаб|лаб\\.|пр|пр\\.|сем|сем\\.)$/.test(s);}" +
                "function isHeader(s){s=clean(s).toLowerCase();return !s||s==='п'||s==='у'||s==='тема'||s.indexOf('выберите')>=0||s.indexOf('журнал')>=0;}" +
                "function badSubject(s){s=clean(s);var low=s.toLowerCase();return isHeader(s)||isDate(s)||dayName(s)||isPair(s)||isMark(s)||isType(s)||low==='дата'||low==='день';}" +
                "function badTheme(s){s=clean(s);return badSubject(s)||s.toLowerCase()==='тема';}" +
                ...
                "var raw=cells.join(' | ');" +
                "if(raw.toLowerCase().indexOf('выберите')>=0)return;" +
                ...
                "for(var i=pairIndex+1;i<cells.length;i++){var c=cells[i];if(!badSubject(c)&&/[А-Яа-яA-Za-z]/.test(c)){subject=c;subjectIndex=i;break;}}" +
                "if(!subject)return;" +
                ...
                "var theme='';" +
                "for(var i=cells.length-1;i>subjectIndex;i--){var tc=cells[i];if(i===typeIndex||i===markIndex)continue;if(!badTheme(tc)&&tc!==subject&&/[А-Яа-яA-Za-z]/.test(tc)){theme=tc;break;}}" +
                "var mark=marks.join(', ');" +
                "var attendance='';if(/нн|н/i.test(mark))attendance=mark.toUpperCase();" +
    ...
    private void showGradesRows(JSONArray rows) {
        ...
        header.setText("Оценки и посещаемость");
        ...
        selected.setText("Семестр: " + (selectedGradesSemester.isEmpty() ? "не выбран" : selectedGradesSemester)
                + "\nНеделя: " + (selectedGradesWeek.isEmpty() ? "не выбрана" : selectedGradesWeek));
        selected.setTextColor(textMain);
        selected.setTextSize(14);
        selected.setTypeface(Typeface.DEFAULT_BOLD);
        selected.setPadding(dp(10), dp(8), dp(10), dp(8));
        selected.setBackground(pillBackground(greenSoft, border));
        LinearLayout.LayoutParams selectedParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        selectedParams.setMargins(0, dp(9), 0, 0);
        summary.addView(selected, selectedParams);

        TextView counts = new TextView(this);
        counts.setText("С оценками: " + withMarks
                + "   •   двоек: " + twos
                + "\nН/НН: " + absences
                + "   •   без оценки: " + noMarks);
        ...
        if (problems > 0) {
            TextView problemsButton = gradesSmallButton("Показать проблемы: 2 / Н / НН (" + problems + ")");
            problemsButton.setOnClickListener(v -> showGradeProblems(rows));
        ...
        if (rows.length() == 0) {
            addGradesMessage("Оценки не найдены",
                    "В таблице не удалось найти строки журнала. Проверь, выбран ли семестр и неделя.");
            return;
        }
    ...
    private void addGradesDayHeader(String day, String date) {
        ...
        if (date != null && !date.isEmpty()) text += (text.isEmpty() ? "" : " • ") + date;
        header.setText(text);
        header.setTextColor(textMain);
        ...
        header.setBackground(pillBackground(greenSoft, border));

        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        params.setMargins(0, dp(12), 0, dp(8));
        gradesResultContent.addView(header, params);
    }
    private void addGradeRowCard(JSONObject row) {
        ...
        mark.setText(markText.isEmpty() ? "нет" : markText.toUpperCase(ru));
        mark.setTextColor(Color.WHITE);
        mark.setTextSize(markText.length() <= 2 ? 20 : 15);
        mark.setTypeface(Typeface.DEFAULT_BOLD);
        mark.setGravity(Gravity.CENTER);
        mark.setMinWidth(dp(54));
        mark.setMinimumHeight(dp(42));
        mark.setPadding(dp(13), dp(7), dp(13), dp(7));
        ...
        addGradeChip(meta, row.optString("lesson", "").isEmpty()
                ? "Пара: —"
                : "Пара: " + row.optString("lesson", ""));

        addGradeLetterBadge(meta, row.optString("type", ""));
        addGradeLetterBadge(meta, row.optString("attendance", ""));
        ...
        details.addView(gradesInfoLine("Оценка", markText.isEmpty() ? "нет" : markText.toUpperCase(ru)));

        String typeCode = gradeCode(row.optString("type", ""));
        if (!typeCode.isEmpty()) {
            details.addView(gradesInfoLine("Тип", typeCode + " — " + gradeCodeLabel(typeCode)));
        }

        String attendanceCode = gradeCode(row.optString("attendance", ""));
        if (!attendanceCode.isEmpty()) {
            details.addView(gradesInfoLine("Посещение", attendanceCode + " — " + gradeCodeLabel(attendanceCode)));
        }

        String theme = row.optString("theme", "").trim();
        details.addView(gradesInfoLine("Тема", theme.isEmpty() ? "не указана" : theme));

        if (!row.optString("date", "").isEmpty()) {
            details.addView(gradesInfoLine("Дата", row.optString("date", "")));
        }
    ...
    private boolean hasGradesSelection() {
        return (selectedGradesSemester != null && !selectedGradesSemester.trim().isEmpty())
                || (selectedGradesWeek != null && !selectedGradesWeek.trim().isEmpty());
    }

    private void updateGradesResetSelectionButton() {
        if (gradesResetSelectionButton == null) return;

        boolean shouldShow = hasGradesSelection();

        if (shouldShow == gradesResetSelectionButtonVisible) {
            gradesResetSelectionButton.setVisibility(shouldShow ? View.VISIBLE : View.GONE);
            gradesResetSelectionButton.setAlpha(shouldShow ? 1.0f : 0.0f);
            return;
        }

        gradesResetSelectionButtonVisible = shouldShow;

        if (shouldShow) {
            gradesResetSelectionButton.setVisibility(View.VISIBLE);
            gradesResetSelectionButton.setAlpha(0.0f);
            gradesResetSelectionButton.setTranslationY(dp(8));
            gradesResetSelectionButton.animate()
                    .alpha(1.0f)
                    .translationY(0)
                    .setDuration(180)
                    .start();
            ...
            gradesResetSelectionButton.animate()
                    ...
                    .setDuration(160)
                    .withEndAction(() -> {
                        if (!hasGradesSelection() && gradesResetSelectionButton != null) {
                            gradesResetSelectionButton.setVisibility(View.GONE);
                        }
                    })
                    .start();
        }
    ...
    private void toggleGradesJournal(TextView button) {
        ensureGradesWebView();

        if (gradesJournalVisible) {
            gradesJournalVisible = false;
            button.animate()
                    .alpha(0.65f)
                    .setDuration(90)
                    .withEndAction(() -> {
                        button.setText("Открыть журнал");
                        attachGradesWebView(dp(1));
                        button.animate().alpha(1.0f).setDuration(120).start();
                    })
                    .start();

            addGradesMessage("Журнал скрыт",
                    "Окно журнала закрыто. Данные и выбранные значения остались на месте.");
            return;
        }

        gradesJournalVisible = true;
        button.animate()
                .alpha(0.65f)
                .setDuration(90)
                .withEndAction(() -> {
                    button.setText("Закрыть журнал");
                    attachGradesWebView(dp(520));
                    button.animate().alpha(1.0f).setDuration(120).start();
                })
                .start();
        ...
        gradesWebView.loadUrl(DEKANAT_JOURNAL_URL);
        addGradesMessage("Открываю журнал",
                "Открываю страницу журнала в видимом окне внутри приложения...");
    }

    private String gradeTypeLabel(String type) {
        if (type == null) return "";

        String t = type.trim().toUpperCase(ru);

        if (t.equals("П")) return "Присутствовал";
        if (t.equals("О")) return "Онлайн";
        if (t.equals("Н")) return "Пропуск";
        if (t.equals("НН")) return "Двойной пропуск";
    ...
    private void resetGradesSelection() {
        ...
        if (gradesResultContent != null) {
            addGradesMessage("Выбор сброшен",
                    "Семестр, неделя и прошлый список оценок очищены. Теперь выбери семестр и неделю заново.");
        }
        ...
        if (gradesWebView != null) {
            String js = "(function(){" +
                    ...
                    "function isLangText(t){t=txt(t).toLowerCase();return t==='ru'||t==='rus'||t==='рус'||t==='en'||t==='eng'||t==='english'||t.indexOf('русский')>=0||t.indexOf('англ')>=0;}" +
                    "function isLanguageSelect(s){var os=[].slice.call(s.options).map(txt).filter(Boolean);return os.length>0&&os.every(isLangText);}" +
                    "var selects=[].slice.call(document.querySelectorAll('select')).filter(function(s){return !isLanguageSelect(s);});" +
                    "selects.forEach(function(s){s.selectedIndex=0;s.dispatchEvent(new Event('input',{bubbles:true}));s.dispatchEvent(new Event('change',{bubbles:true}));});" +
                    "return true;" +
                    "})()";
    ...
    private void showProblemsFromLastGrades() {
        if (gradesLastRows == null || gradesLastRows.length() == 0) {
            addGradesMessage("Сначала собери оценки",
                    "Нажми «Собрать оценки», дождись списка, а потом открывай «2 / Н / НН». Так будет использоваться уже правильно собранная таблица.");
            return;
        }
    ...
    private void showGradesRawDiagnostics(JSONArray rows) {
        if (gradesResultContent == null) return;
        gradesResultContent.removeAllViews();
        ...
        TextView title = new TextView(this);
        title.setText("Диагностика строк журнала");
        title.setTextColor(textMain);
        title.setTextSize(20);
        title.setTypeface(Typeface.DEFAULT_BOLD);
        summary.addView(title);

        TextView back = gradesSmallButton("Вернуться ко всем оценкам");
        ...
        gradesResultContent.addView(summary);

        int limit = Math.min(rows == null ? 0 : rows.length(), 30);
    ...
    private void showAllGradeProblemsResult() {
        if (gradesResultContent == null) return;
        gradesResultContent.removeAllViews();
        ...
        TextView subtitle = new TextView(this);
        subtitle.setText("Семестр: " + (selectedGradesSemester.isEmpty() ? "не выбран" : selectedGradesSemester)
                + "\nПроверено недель: " + gradesProblemTotalWeeks
                + "\nНайдено проблем: " + gradesProblemRows.length());
        subtitle.setTextColor(textMuted);
        subtitle.setTextSize(13);
        subtitle.setLineSpacing(dp(2), 1.0f);
        LinearLayout.LayoutParams subtitleParams = new LinearLayout.LayoutParams(
        ...
        TextView rescan = gradesSmallButton("Пересобрать проблемы");
        rescan.setOnClickListener(v -> collectAllGradeProblems());
        LinearLayout.LayoutParams rescanParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        rescanParams.setMargins(0, dp(10), 0, 0);
        summary.addView(rescan, rescanParams);
        ...
        if (gradesProblemRows.length() == 0) {
            addGradesMessage("Проблем нет", "Двоек, Н и НН по выбранному семестру не найдено.");
            return;
        }
        ...
        for (int i = 0; i < gradesProblemRows.length(); i++) {
            try {
                JSONObject row = gradesProblemRows.getJSONObject(i);
                String week = row.optString("week", "");
                String day = row.optString("day", "");
                String date = row.optString("date", "");

                if (!week.equals(lastWeek)) {
                    addProblemsWeekHeader(week);
                    lastWeek = week;
                    lastDay = "";
                    lastDate = "";
                }
                ...
                addProblemRowCard(row);
            } catch (Exception ignored) {
            }
        }
    }

    private void addProblemsWeekHeader(String week) {
        TextView header = new TextView(this);
        header.setText(week == null || week.trim().isEmpty() ? "Неделя" : week.trim());
        header.setTextColor(Color.WHITE);
        header.setTextSize(15);
        header.setTypeface(Typeface.DEFAULT_BOLD);
        header.setGravity(Gravity.CENTER_VERTICAL);
        header.setPadding(dp(12), dp(10), dp(12), dp(10));
        header.setBackground(pillBackground(green, green));
    ...
    private void showGradeProblems(JSONArray rows) {
        ...
        TextView title = new TextView(this);
        title.setText("2 / Н / НН из собранных оценок");
        title.setTextColor(textMain);
        title.setTextSize(21);
        title.setTypeface(Typeface.DEFAULT_BOLD);
        summary.addView(title);
        ...
        TextView subtitle = new TextView(this);
        subtitle.setText("Здесь только проблемы из уже собранного списка оценок. То есть берём те же данные, которые показались правильно.");
        subtitle.setTextColor(textMuted);
        subtitle.setTextSize(13);
        subtitle.setLineSpacing(dp(2), 1.0f);
        LinearLayout.LayoutParams subtitleParams = new LinearLayout.LayoutParams(
        ...
        TextView back = gradesSmallButton("Показать все оценки");
        back.setOnClickListener(v -> showGradesRows(rows));
        LinearLayout.LayoutParams backParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        backParams.setMargins(0, dp(10), 0, 0);
        ...
        if (count == 0) {
            addGradesMessage("Проблем нет", "Двоек, Н и НН на выбранной неделе не найдено.");
        }
    }
    ...
    private boolean isProblemRow(JSONObject row) {
        String mark = problemMarkFromRow(row);
        return isProblemGrade(mark);
    }

    private boolean isProblemGrade(String mark) {
        String m = mark == null ? "" : mark.toLowerCase(ru).trim();
        return m.contains("2") || m.contains("нн") || m.equals("н") || m.contains(", н") || m.startsWith("н,");
    }



    private String displayGradeValue(JSONObject row) {
        String mark = row.optString("mark", "").trim();
        if (!mark.isEmpty()) return mark;

        return problemMarkFromRow(row);
    }

    private String problemMarkFromRow(JSONObject row) {
        String mark = row.optString("mark", "").trim();
        if (!mark.isEmpty()) return mark;

        String attendance = row.optString("attendance", "").trim().toUpperCase(ru);
        if (attendance.equals("Н") || attendance.equals("НН")) return attendance;

        String fromRaw = problemMarkFromRawColumns(row);
    ...
    private String problemMarkFromRawColumns(JSONObject row) {
        String raw = row.optString("raw", "");
        String cells = row.optString("cells", "");
        String subject = row.optString("subject", "");

        String source = !cells.trim().isEmpty() ? cells.replace(" || ", " | ") : raw;
        ...
        if (subject != null && !subject.trim().isEmpty()) {
            for (int i = 0; i < parts.length; i++) {
                if (parts[i].trim().equalsIgnoreCase(subject.trim())) {
                    startIndex = i + 1;
                    break;
                }
            }
        ...
        for (int i = startIndex; i < parts.length; i++) {
            ...
            String found = exactProblemToken(candidate);
    ...
    private String exactProblemToken(String value) {
        if (value == null) return "";
        ...
        for (String tokenRaw : tokens) {
            String token = normalizeGradePart(tokenRaw);

            if (token.equals("НН")) return "НН";
            if (token.equals("Н")) return "Н";
            if (token.equals("2")) return "2";
            if (token.equalsIgnoreCase("незач")) return "незач";
        }

        return "";
    }

    private String normalizeGradePart(String value) {
        if (value == null) return "";
        ...
        if (token.equals("н")) return "Н";
        if (token.equals("нн")) return "НН";
        if (token.equals("з")) return "зач";
        return token;
    }

    private String attendanceCode(JSONObject row) {
        String mark = problemMarkFromRow(row).trim().toUpperCase(ru);
        if (mark.contains("НН")) return "НН";
        if (mark.equals("Н") || mark.contains(", Н") || mark.startsWith("Н,")) return "Н";

        String type = row.optString("type", "").trim().toUpperCase(ru);
        if (type.equals("П") || type.equals("О")) return type;

        return "";
    }

    private int attendanceColor(String code) {
        if (code == null) return green;
        String c = code.trim().toUpperCase(ru);
        if (c.equals("Н") || c.equals("НН")) return Color.rgb(198, 80, 72);
        if (c.equals("О")) return Color.rgb(74, 105, 170);
    ...
    private void addAttendanceStatusCard(LinearLayout card, JSONObject row) {
        String code = attendanceCode(row);
        if (code.isEmpty()) return;
        ...
        badge.setTypeface(Typeface.DEFAULT_BOLD);
        badge.setGravity(Gravity.CENTER);
        badge.setMinWidth(dp(38));
        badge.setPadding(dp(9), dp(5), dp(9), dp(5));
        badge.setBackground(pillBackground(attendanceColor(code), attendanceColor(code)));
        box.addView(badge);
    ...
    private void addGradesMessageNoAnimation(String title, String message) {
        addGradesMessage(title, message);
    }
    ...
    private void addGradesSelectionStatus() {
        LinearLayout card = settingsCard();
        card.setOrientation(LinearLayout.VERTICAL);
        ...
        marker.setTypeface(Typeface.DEFAULT_BOLD);
        marker.setGravity(Gravity.CENTER);
        marker.setBackground(pillBackground(hasGradesSelection() ? green : border, hasGradesSelection() ? green : border));
        LinearLayout.LayoutParams markerParams = new LinearLayout.LayoutParams(dp(30), dp(30));
        markerParams.setMargins(0, 0, dp(10), 0);
        header.addView(marker, markerParams);

        TextView title = new TextView(this);
        title.setText("Выбрано сейчас");
        title.setTextColor(textMain);
        ...
        status.setText(hasGradesSelection() ? "активно" : "пусто");
        status.setTextColor(hasGradesSelection() ? greenDark : textMuted);
        status.setTextSize(11);
        status.setTypeface(Typeface.DEFAULT_BOLD);
        status.setGravity(Gravity.CENTER);
        status.setPadding(dp(9), dp(5), dp(9), dp(5));
        status.setBackground(pillBackground(hasGradesSelection() ? greenSoft : cardSurface, border));
        header.addView(status);

        TextView hint = new TextView(this);
        hint.setText("Сначала выбери семестр, затем неделю. Текущий выбор показан ниже.");
        hint.setTextColor(textMuted);
        hint.setTextSize(12);
        hint.setLineSpacing(dp(2), 1.0f);
        LinearLayout.LayoutParams hintParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        hintParams.setMargins(0, dp(7), 0, dp(12));
        card.addView(hint, hintParams);

        LinearLayout selectionBox = new LinearLayout(this);
        selectionBox.setOrientation(LinearLayout.VERTICAL);
        selectionBox.setPadding(dp(10), dp(10), dp(10), dp(10));
        selectionBox.setBackground(pillBackground(greenSoft, border));
        card.addView(selectionBox, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT));

        gradesSelectedSemesterView = gradesSelectionTile("Семестр", selectedGradesSemester);
        selectionBox.addView(gradesSelectedSemesterView, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT));

        gradesSelectedWeekView = gradesSelectionTile("Неделя", selectedGradesWeek);
        LinearLayout.LayoutParams weekParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        weekParams.setMargins(0, dp(8), 0, 0);
        selectionBox.addView(gradesSelectedWeekView, weekParams);

        gradesResetSelectionButton = gradesSmallButton("Сбросить выбор");
        gradesResetSelectionButton.setOnClickListener(v -> resetGradesSelection());
        gradesResetSelectionButton.setVisibility(hasGradesSelection() ? View.VISIBLE : View.GONE);
        gradesResetSelectionButton.setAlpha(hasGradesSelection() ? 1.0f : 0.0f);
        gradesResetSelectionButtonVisible = hasGradesSelection();

        LinearLayout.LayoutParams resetParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        resetParams.setMargins(0, dp(10), 0, 0);
        card.addView(gradesResetSelectionButton, resetParams);

        addGradesCard(card);
    }
    private TextView gradesSelectionTile(String label, String value) {
        TextView view = new TextView(this);
        String shown = value == null || value.trim().isEmpty() ? "не выбрано" : value.trim();
        view.setText(label + "\n" + shown);
        ...
        view.setBackground(locationBackground(cardSurface, border));
        view.setSingleLine(false);
        view.setLineSpacing(dp(3), 1.0f);
        return view;
    }
    private void refreshGradesSelectionViews() {
        if (gradesSelectedSemesterView != null) {
            gradesSelectedSemesterView.setText("Семестр\n" + (selectedGradesSemester == null || selectedGradesSemester.trim().isEmpty()
                    ? "не выбрано"
                    : selectedGradesSemester.trim()));
        }

        if (gradesSelectedWeekView != null) {
            gradesSelectedWeekView.setText("Неделя\n" + (selectedGradesWeek == null || selectedGradesWeek.trim().isEmpty()
                    ? "не выбрано"
                    : selectedGradesWeek.trim()));
        }

        updateGradesResetSelectionButton();
    }




    private TextView gradesInfoLine(String left, String right) {
        TextView view = new TextView(this);
        String value = right == null || right.trim().isEmpty() ? "—" : right.trim();
        view.setText(left + ": " + value);
    ...
    private void addProblemRowCard(JSONObject row) {
        LinearLayout card = settingsCard();
        card.setOrientation(LinearLayout.VERTICAL);
        ...
        addGradeChip(chips, row.optString("lesson", "").isEmpty()
                ? "пара —"
                : row.optString("lesson", "") + " пара");
        if (!row.optString("type", "").isEmpty()) {
            addGradeChip(chips, "Формат: " + row.optString("type", ""));
        }

        String details = "";
        if (!row.optString("date", "").isEmpty()) details += "Дата: " + row.optString("date", "");
        if (!row.optString("theme", "").isEmpty()) {
            details += (details.isEmpty() ? "" : "\n") + "Тема: " + row.optString("theme", "");
        }

        TextView info = new TextView(this);
        info.setText(details.isEmpty() ? "Информация по занятию не указана" : details);
        info.setTextColor(textMuted);
        info.setTextSize(13);
        info.setLineSpacing(dp(2), 1.0f);
        LinearLayout.LayoutParams infoParams = new LinearLayout.LayoutParams(
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.19.20");
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (!isProbablyApkFile(apkFile)) {
                    ...
                    throw new Exception("GitHub отдал не APK-файл. Проверь asset в Release: нужен именно app-debug.apk, а не zip/html.");
                }
