...
public class MainActivity extends Activity {
    ...
    private void addGradesLoginForm() {
        ...
        note.setText("Сайт БелГУ часто завершает сессию примерно через 12 часов. Поэтому можно включить «Запомнить пароль на устройстве»: пароль хранится не открытым текстом, а в зашифрованном виде через Android Keystore.");
        ...
        keepSessionCheck.setText("Не выходить из системы");
        ...
        keepSessionCheck.setPadding(dp(4), dp(2), dp(4), 0);
        addGradesView(keepSessionCheck, 5);
    ...
    private void addGradesTools() {
        ...
        TextView problems = gradesSmallButton("Показать 2 / Н / НН");
    ...
    private String buildGetWeekOptionsScript() {
        return "(function(){" +
                ...
                "return JSON.stringify([].slice.call(best.options).map(function(o,i){return {index:i,text:txt(o)||('Неделя '+(i+1))};}).filter(function(x){var low=x.text.toLowerCase();return x.text&&low.indexOf('выберите')<0&&!/^\\d{1,2}$/.test(x.text);}));" +
                "})()";
    }
    ...
    private void setGradesOption(boolean semester, int index, String label) {
        String js = "(function(){" +
                ...
                "var target=null;" +
                (semester
                        ? "target=selects.find(function(s){return isSemesterSelect(s);})||selects[0];"
    ...
    private void collectCurrentWeekIntoManualSemester(int attempt) {
        ...
        gradesWebView.evaluateJavascript(buildCollectGradesScript(), rowsValue -> {
            try {
                ...
                Toast.makeText(this,
                        "Неделя добавлена. Всего записей: " + gradesSemesterRows.length(),
    ...
    private void showGradesRows(JSONArray rows) {
        ...
        counts.setText("Всего записей: " + rows.length()
                + "\nС оценками: " + withMarks
                + "   •   двоек: " + twos
                + "\nН/НН: " + absences
                + "   •   без оценки: " + noMarks);
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.19.12");
