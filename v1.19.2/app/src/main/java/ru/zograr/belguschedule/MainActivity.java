...
public class MainActivity extends Activity {
    ...
    private String buildCollectGradesScript() {
        return "(function(){" +
                ...
                "function isMarkToken(t){t=normToken(t);return /^(нн|н|2|3|4|5|зач|незач|диффзач|з)$/.test(t);}" +
                "function isPresenceToken(t){t=normToken(t);return /^(п|о)$/.test(t);}" +
                "function isLessonTypeToken(t){t=normToken(t);return /^(л|лек|лаб|пр|сем)$/.test(t);}" +
                "function normalizeMark(t){t=normToken(t);if(t==='н'||t==='нн')return t.toUpperCase();if(t==='з')return 'зач';return t;}" +
                "function isHeader(s){s=clean(s).toLowerCase();return !s||s==='п'||s==='о'||s==='у'||s==='тема'||s.indexOf('выберите')>=0||s.indexOf('журнал')>=0;}" +
                "function badSubject(s){s=clean(s);var low=s.toLowerCase();var toks=splitTokens(s);return isHeader(s)||isDate(s)||dayName(s)||isPair(s)||toks.some(isMarkToken)||toks.some(isPresenceToken)||toks.some(isLessonTypeToken)||low==='дата'||low==='день';}" +
                ...
                "if(!subject)return;" +
                "var marks=[];var attendance='';var presence='';var lessonType='';" +
                "for(var i=subjectIndex+1;i<cells.length;i++){" +
                "var toks=splitTokens(cells[i]);" +
                "for(var ti=0;ti<toks.length;ti++){" +
                "var t=toks[ti];" +
                "if(isMarkToken(t)){var nm=normalizeMark(t);marks.push(nm);if(nm==='Н'||nm==='НН')attendance=nm;}" +
                "else if(isPresenceToken(t)&&!presence){presence=t.toUpperCase();}" +
                "else if(isLessonTypeToken(t)&&!lessonType){lessonType=t;}" +
                "}" +
                "}" +
                "marks=unique(marks);" +
                "var theme='';" +
                ...
                "var mark=marks.join(', ');" +
                "var status=attendance||presence;" +
                "out.push({day:currentDay,date:currentDate,lesson:cells[pairIndex],subject:subject,type:lessonType,mark:mark,attendance:status,theme:theme,raw:raw});" +
                "});" +
    ...
    private void addGradeRowCard(JSONObject row) {
        ...
        String attendanceText = attendanceDisplayText(row);
        if (!attendanceText.isEmpty()) {
            addGradeChip(meta, attendanceText);
        }
    ...
    private String firstProblemOrGradeToken(String value) {
        ...
        for (String part : parts) {
            ...
            if (token.equals("2") || token.equals("3") || token.equals("4") || token.equals("5")
                    || token.equals("Н") || token.equals("НН")
                    || token.equalsIgnoreCase("зач") || token.equalsIgnoreCase("незач")) {
                return token;
            }
        }
    ...
    private String attendanceDisplayText(JSONObject row) {
        String attendance = row.optString("attendance", "").trim().toLowerCase(ru);
        if (attendance.equals("п")) return "присутствовал";
        if (attendance.equals("о")) return "онлайн";
        if (attendance.equals("н")) return "пропуск: Н";
        if (attendance.equals("нн")) return "пропуск: НН";
        return "";
    }
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.19.2");
