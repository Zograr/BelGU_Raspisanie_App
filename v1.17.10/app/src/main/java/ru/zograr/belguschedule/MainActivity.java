...
public class MainActivity extends Activity {
            ...
            try {
                ...
                if (options.length() == 0) {
                    addGradesMessage("Список не найден", semester
                            ...
                            : "Не нашёл список недель на странице. Сначала выбери семестр и подожди пару секунд, пока сайт загрузит недели.");
                    return;
                }
    ...
    private String buildGetWeekOptionsScript() {
        return "(function(){" +
                ...
                "function isWeekText(t){t=txt(t);return /\\d{2}\\.\\d{2}\\.\\d{4}/.test(t)&&/[-–—]/.test(t);}" +
                "var selects=[].slice.call(document.querySelectorAll('select'));" +
                "var target=selects.find(function(s){var os=[].slice.call(s.options);return os.filter(function(o){return isWeekText(txt(o));}).length>=1;});" +
                "if(!target)return JSON.stringify([]);" +
                "return JSON.stringify([].slice.call(target.options).map(function(o,i){return {index:i,text:txt(o)||('Неделя '+(i+1))};}).filter(function(x){return x.text&&x.text.toLowerCase().indexOf('выберите')<0&&isWeekText(x.text);}));" +
                "})()";
    }
    ...
    private void setGradesOption(boolean semester, int index, String label) {
        String js = "(function(){" +
                ...
                (semester
                        ...
                        : "target=selects.find(function(s){var os=[].slice.call(s.options);return os.filter(function(o){return isWeekText(txt(o));}).length>=1;});") +
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.17.10.100.9.8.7.6.5.5.4.3.2");
