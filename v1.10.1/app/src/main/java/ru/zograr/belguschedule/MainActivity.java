...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        refreshButton.setText("Проверить изменения");
...
private void showSettingsDialog() {
    ...
    hero.setBackground(cardBackground(cardSurface));
    box.addView(hero);
    ...
    subtitle.setText("Группа " + scheduleGroup + " • " + subgroupDisplay()
            + "\nСейчас: " + (showWholeWeek ? "вся неделя" : "только сегодня"));
    ...
    box.addView(settingsSectionTitle("Расписание"));

    box.addView(settingsItem(
            "Режим показа",
            showWholeWeek ? "Показывать всю неделю в компактном виде" : "Показывать только пары на сегодня",
            showWholeWeek ? "Неделя" : "Сегодня",
            v -> {
                ...
                Toast.makeText(this, showWholeWeek ? "Включен показ всей недели" : "Включен показ на сегодня", Toast.LENGTH_SHORT).show();
            }
    ));

    box.addView(settingsItem(
            "Подгруппа",
            "Выбери 1, 2 или показ всех подгрупп",
            subgroupDisplayShort(),
            v -> showSubgroupDialog()
    ));

    box.addView(settingsItem(
            "Изменить группу",
            "Можно быстро переключить номер группы",
            scheduleGroup,
            v -> showGroupDialog()
    ));

    box.addView(settingsSectionTitle("Приложение"));

    box.addView(settingsItem(
            "Тема",
            "Светлая или тёмная тема приложения",
    ...
    box.addView(settingsItem(
            "Проверка обновлений",
            "Проверить наличие новой версии приложения на GitHub",
            "Проверить",
            v -> checkForAppUpdatesManually()
    ));

    box.addView(settingsItem(
            "Сайт расписания",
            "Открыть исходный сайт БелГУ в браузере",
            "Открыть",
            v -> openInBrowser()
    ));
...
private LinearLayout settingsItem(String title, String subtitle, String value, View.OnClickListener listener) {
    ...
    subView.setLineSpacing(dp(1), 1.0f);
    LinearLayout.LayoutParams subParams = new LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
    subParams.setMargins(0, dp(4), 0, 0);
    textBox.addView(subView, subParams);

    card.addView(textBox, new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1));

    TextView valueChip = new TextView(this);
    valueChip.setText(value);
    valueChip.setTextColor(greenDark);
    valueChip.setTextSize(13);
    valueChip.setTypeface(Typeface.DEFAULT_BOLD);
    valueChip.setPadding(dp(12), dp(6), dp(12), dp(6));
    valueChip.setBackground(pillBackground(greenSoft, border));
    LinearLayout.LayoutParams valueParams = new LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
    valueParams.setMargins(dp(10), 0, 0, 0);
    card.addView(valueChip, valueParams);
...
private void showSubgroupDialog() {
    ...
    private void showWholeWeekSchedule(ScheduleResult result) {
        ...
        summaryView.setText(weekSummaryText(total) + " • " + subgroupDisplay() + " • режим: неделя");
...
private void addDayBlock(DaySchedule day, List<String> pairs) {
    ...
    String label = day.dayName + " • " + day.displayDate;
    if (todayString().equals(day.displayDate)) label += " • сегодня";
    dayTitle.setText(label);
    dayTitle.setTextColor(textMain);
    dayTitle.setTextSize(17);
    dayTitle.setTypeface(Typeface.DEFAULT_BOLD);
    dayCard.addView(dayTitle);

    TextView dayInfo = new TextView(this);
    dayInfo.setText(summaryText(pairs.size(), day.dayName));
    dayInfo.setTextColor(textMuted);
    dayInfo.setTextSize(13);
    LinearLayout.LayoutParams infoParams = new LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
    infoParams.setMargins(0, dp(4), 0, dp(8));
    dayCard.addView(dayInfo, infoParams);
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private void checkForAppUpdatesManually() {
