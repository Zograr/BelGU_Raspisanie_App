...
public class MainActivity extends Activity {
    ...
    private Button subgroupAllButton;
    private Button subgroup1Button;
    private Button subgroup2Button;
    ...
    private final int greenDark = Color.rgb(8, 73, 20);
    private final int green = Color.rgb(31, 121, 50);
    private final int greenSoft = Color.rgb(231, 245, 234);
    private final int background = Color.rgb(244, 247, 244);
    private final int textMain = Color.rgb(28, 32, 29);
    private final int textMuted = Color.rgb(95, 105, 98);
    private final int border = Color.rgb(223, 231, 224);

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        scheduleUrl = getString(R.string.schedule_url);
        selectedSubgroup = getSharedPreferences(PREFS, MODE_PRIVATE).getString(KEY_SUBGROUP, SUBGROUP_ALL);
        buildUi();
        setupWebView();
        loadCachedSchedule();
        refreshSchedule();
    ...
    private void buildUi() {
        getWindow().setStatusBarColor(greenDark);
        ...
        groupPill.setText("Группа 90002595");
        ...
        header.addView(groupPill, wrapParams());
        ...
        openSiteButton.setBackground(buttonBackground(Color.WHITE, border));
        ...
        LinearLayout subgroupRow = new LinearLayout(this);
        subgroupRow.setOrientation(LinearLayout.HORIZONTAL);
        subgroupRow.setGravity(Gravity.CENTER_VERTICAL);
        subgroupRow.setPadding(dp(12), 0, dp(12), dp(10));
        root.addView(subgroupRow, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT));

        TextView subgroupTitle = new TextView(this);
        subgroupTitle.setText("Подгруппа:");
        subgroupTitle.setTextColor(textMuted);
        subgroupTitle.setTextSize(14);
        subgroupTitle.setTypeface(Typeface.DEFAULT_BOLD);
        LinearLayout.LayoutParams titleParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        titleParams.setMargins(0, 0, dp(8), 0);
        subgroupRow.addView(subgroupTitle, titleParams);

        subgroup1Button = createSubgroupButton("1");
        subgroup2Button = createSubgroupButton("2");
        subgroupAllButton = createSubgroupButton("Все");

        subgroupRow.addView(subgroup1Button, subgroupButtonParams(false));
        subgroupRow.addView(subgroup2Button, subgroupButtonParams(false));
        subgroupRow.addView(subgroupAllButton, subgroupButtonParams(true));

        subgroup1Button.setOnClickListener(v -> selectSubgroup(SUBGROUP_1));
        subgroup2Button.setOnClickListener(v -> selectSubgroup(SUBGROUP_2));
        subgroupAllButton.setOnClickListener(v -> selectSubgroup(SUBGROUP_ALL));
        updateSubgroupUi();
    ...
    private Button createSubgroupButton(String text) {
    ...
    private LinearLayout.LayoutParams subgroupButtonParams(boolean last) {
    ...
    private void updateSubgroupUi() {
        if (groupPill != null) {
            groupPill.setText("Группа 90002595 • " + subgroupDisplay());
        }
        updateSubgroupButton(subgroup1Button, SUBGROUP_1.equals(selectedSubgroup));
        updateSubgroupButton(subgroup2Button, SUBGROUP_2.equals(selectedSubgroup));
        updateSubgroupButton(subgroupAllButton, SUBGROUP_ALL.equals(selectedSubgroup));
    }

    private void updateSubgroupButton(Button button, boolean selected) {
        if (button == null) return;
        button.setTextColor(selected ? Color.WHITE : greenDark);
        button.setBackground(buttonBackground(selected ? green : Color.WHITE, selected ? green : border));
    }
    ...
    private void addPairCard(String text) {
        ...
        outer.setBackground(cardBackground(Color.WHITE));
        ...
        if (parsed.lines.isEmpty()) {
            ...
            for (String line : parsed.lines) {
                ...
                if (pretty.startsWith("Аудитория:")) color = greenDark;
    ...
    private GradientDrawable headerBackground() {
        GradientDrawable drawable = new GradientDrawable(
                ...
                new int[]{Color.rgb(6, 70, 18), Color.rgb(24, 117, 42)});
