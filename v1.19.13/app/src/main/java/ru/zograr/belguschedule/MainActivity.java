...
public class MainActivity extends Activity {
    ...
    private void addGradesTools() {
        ...
        TextView open = gradesSmallButton("Открыть журнал");
        open.setOnClickListener(v -> {
            ensureGradesWebView();
            attachGradesWebView(dp(520));
            gradesWebView.loadUrl(DEKANAT_JOURNAL_URL);
            addGradesMessage("Открываю журнал", "Открываю страницу журнала в видимом окне внутри приложения...");
        });
        LinearLayout.LayoutParams openParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        row2.addView(open, openParams);
    ...
    private void addGradeRowCard(JSONObject row) {
        ...
        if (!row.optString("type", "").isEmpty()) {
            addGradeChip(meta, "тип: " + row.optString("type", ""));
        }

        if (!row.optString("attendance", "").isEmpty()) {
            addGradeChip(meta, "пропуск: " + row.optString("attendance", ""));
        }
    ...
    private void addGradesSelectionStatus() {
        ...
        hint.setText("Перед сбором проверь, что выбран нужный семестр и неделя.");
        ...
        row.addView(gradesSelectedSemesterView, new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1));

        gradesSelectedWeekView = gradesSelectionTile("Неделя", selectedGradesWeek);
        LinearLayout.LayoutParams weekParams = new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1);
        weekParams.setMargins(dp(8), 0, 0, 0);
        row.addView(gradesSelectedWeekView, weekParams);
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.19.13");
