...
public class MainActivity extends Activity {
    ...
    private void addGradesTools() {
        ...
        TextView sub = new TextView(this);
        sub.setText("Для обычного просмотра выбери семестр и неделю. Для отдельного списка двоек и Н/НН нажми «Собрать все 2 / Н / НН» — приложение пройдёт по неделям и покажет дату, предмет и оценку/пропуск.");
        sub.setTextColor(textMuted);
        sub.setTextSize(13);
        ...
        TextView semester = gradesSmallButton("Выбрать семестр");
        ...
        TextView week = gradesSmallButton("Выбрать неделю");
        ...
        addGradesView(collect, 8);

        TextView collectProblems = gradesSmallButton("Собрать все 2 / Н / НН");
        collectProblems.setOnClickListener(v -> collectAllGradeProblems());
        addGradesView(collectProblems, 8);
    ...
    private String buildCollectProblemGradesScript() {
        return "(function(){" +
                "function clean(s){return ((s||'')+'').replace(/\\u00a0/g,' ').replace(/\\s+/g,' ').trim();}" +
                "function compact(s){return clean(s).replace(/\\s+/g,'').toLowerCase();}" +
                "function dateOf(s){var m=clean(s).match(/\\d{1,2}\\.\\d{1,2}\\.\\d{2,4}/);return m?m[0]:'';}" +
                ...
                "function isProblemMark(s){var c=compact(s);return c==='нн'||c==='н'||c==='2'||c==='2.'||c==='2,0';}" +
                "function problemMark(s){var c=compact(s);if(c.indexOf('нн')>=0)return 'НН';if(c==='н'||c.indexOf(' н ')>=0)return 'Н';if(c==='2'||c==='2.'||c==='2,0')return '2';return '';}" +
                ...
                "function isBadSubject(s){var t=clean(s);var low=t.toLowerCase();return !t||dateOf(t)||dayName(t)||isProblemMark(t)||isPair(t)||isType(t)||low==='тема'||low==='дата'||low==='день'||low.indexOf('выберите')>=0||low.indexOf('журнал')>=0||low.indexOf('семестр')>=0||low.indexOf('неделя')>=0;}" +
                "function subjectFrom(cells){var best='';cells.forEach(function(c){var t=clean(c);if(!isBadSubject(t)&&/[А-Яа-яA-Za-z]/.test(t)&&t.length>best.length){best=t;}});return best;}" +
                "function themeFrom(cells, subject){var best='';cells.forEach(function(c){var t=clean(c);if(!isBadSubject(t)&&t!==subject&&/[А-Яа-яA-Za-z]/.test(t)&&t.length>best.length){best=t;}});return best;}" +
                "var out=[];var currentDate='';var currentDay='';var currentSubject='';" +
                ...
                "var raw=cells.join(' | ');" +
                "var d='';for(var i=0;i<cells.length;i++){d=dateOf(cells[i]);if(d)break;}if(d)currentDate=d;" +
                "var dn='';for(var i=0;i<cells.length;i++){dn=dayName(cells[i]);if(dn)break;}if(dn)currentDay=dn;" +
                "var guessedSubject=subjectFrom(cells);if(guessedSubject&&guessedSubject.length>5)currentSubject=guessedSubject;" +
                "var pair='';for(var i=0;i<cells.length;i++){if(isPair(cells[i])){pair=cells[i];break;}}" +
                "var marks=[];" +
                "for(var i=0;i<cells.length;i++){var m=problemMark(cells[i]);if(!m)continue;if(pair&&cells[i]===pair&&m==='2')continue;marks.push(m);}" +
                "if(marks.length===0){" +
                "var rawLow=(' '+raw.toLowerCase()+' ');if(rawLow.indexOf(' нн ')>=0)marks.push('НН');else if(rawLow.indexOf(' н ')>=0)marks.push('Н');" +
                "}" +
                "if(marks.length===0)return;" +
                "var subject=guessedSubject||currentSubject||'Предмет не определён';" +
                "var theme=themeFrom(cells, subject);" +
                "out.push({day:currentDay,date:d||currentDate,lesson:pair,subject:subject,type:'',mark:marks.join(', '),attendance:marks.join(', '),theme:theme,raw:raw});" +
                "});" +
    ...
    private void setWeekByIndexForProblems(int index, String label) {
        ...
        gradesWebView.evaluateJavascript(js, value -> main.postDelayed(() -> {
            gradesWebView.evaluateJavascript(buildCollectProblemGradesScript(), rowsValue -> {
                try {
    ...
    private void showAllGradeProblemsResult() {
        ...
        subtitle.setText("Семестр: " + (selectedGradesSemester.isEmpty() ? "не выбран" : selectedGradesSemester)
                ...
                + "\nСписок ниже показывает: когда, какой предмет и какая оценка/пропуск.");
        subtitle.setTextColor(textMuted);
        subtitle.setTextSize(13);
    ...
    private void addProblemRowCard(JSONObject row) {
        ...
        mark.setTextSize(markText.length() <= 2 ? 21 : 15);
        mark.setTypeface(Typeface.DEFAULT_BOLD);
        mark.setGravity(Gravity.CENTER);
        mark.setMinWidth(dp(58));
        ...
        top.addView(mark, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT));

        LinearLayout titleBox = new LinearLayout(this);
        titleBox.setOrientation(LinearLayout.VERTICAL);
        LinearLayout.LayoutParams titleBoxParams = new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1);
        titleBoxParams.setMargins(dp(12), 0, 0, 0);
        ...
        String dateText = row.optString("date", "").isEmpty() ? "дата не указана" : row.optString("date", "");
        if (!row.optString("day", "").isEmpty()) dateText = row.optString("day", "") + " • " + dateText;
        dateLine.setText(dateText);
        dateLine.setTextColor(textMuted);
        dateLine.setTextSize(13);
        LinearLayout.LayoutParams dateParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        dateParams.setMargins(0, dp(3), 0, 0);
        titleBox.addView(dateLine, dateParams);
        ...
        if (!row.optString("week", "").isEmpty()) {
            addGradeChip(chips, "неделя");
        }

        String theme = row.optString("theme", "").trim();
        TextView info = new TextView(this);
        info.setText(theme.isEmpty() ? "Тема: не указана" : "Тема: " + theme);
        info.setTextColor(textMuted);
        info.setTextSize(13);
        info.setLineSpacing(dp(2), 1.0f);
        LinearLayout.LayoutParams infoParams = new LinearLayout.LayoutParams(
    ...
    private void showGradeProblems(JSONArray rows) {
        ...
        TextView subtitle = new TextView(this);
        subtitle.setText("Здесь собраны только двойки и пропуски. Нажми кнопку ниже, чтобы вернуться ко всем оценкам.");
        subtitle.setTextColor(textMuted);
        subtitle.setTextSize(13);
        subtitle.setLineSpacing(dp(2), 1.0f);
        LinearLayout.LayoutParams subtitleParams = new LinearLayout.LayoutParams(
...
private void addBodyLine(LinearLayout parent, String text, boolean important, int color) {
    ...
    private UpdateInfo fetchLatestRelease(String owner, String repo) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "BelGUScheduleApp/1.17.13.133.122.111.100.9.8.7.6.5.5.4.3.2");
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (!isValidApkFile(apkFile)) {
                    ...
                    throw new Exception("GitHub отдал не APK или файл скачался повреждённым. Удали asset в Release и загрузи настоящий app-debug.apk.");
                }
    ...
    private void openApkInstaller(Uri apkUri) {
        try {
            ...
            try {
                ...
                showDownloadOpenError("Не удалось открыть системный установщик APK. Если через браузер APK ставится, значит текущая установленная версия ещё отдаёт файл установщику неправильно. Поставь этот хотфикс вручную один раз.");
            }
        }
    }
